	...
	MPOINT ptStart, MPOINT ptEnd, MByte Mask, MLong *pPtNum)
{
	...
	{
		x = MIN(MAX(ptStart.x, 0), lWidth-1);
		pTmpRlt = pData + x;
	...
	{
		y = MIN(MAX(ptStart.y, 0), lHeight-1);
		pTmpRlt = pData + y*lDataLine;
	...
	MPOINT ptStart, MPOINT ptEnd, MByte Mask, MLong *pPtNum, MLong *lNum,MLong lCircleDist)
{
	...
	{
		y = MIN(MAX(ptStart.y, 0), lHeight-1);
		pTmpRlt = pData + ptStart.y*lDataLine;
		lStart = MIN(MAX(MIN(ptStart.x, ptEnd.x), 0),lWidth-1);
		lEnd = MIN(MAX(MAX(ptStart.x, ptEnd.x), 0),lWidth-1);
		for (x=lStart; x<=lEnd;x++)
		{
...
//			MDouble Weight = GaussianWeight(lDist/5, ABS(y-lMid));
		...
		if (pPtWeightSum!=MNull)
			*pPtWeightSum=weightSum;
	...
	MPOINT ptStart, MPOINT ptEnd, MByte Mask, MLong *pPtNum)
{
	...
	if (ptStart.y==ptEnd.y)
	{
		y = MIN(MAX(ptStart.y, 0), lHeight-1);
		pTmpRlt = pData + y*lDataLine;
		pWeightRlt = pWeightData + y*lDataLine;
		lStart = MIN(MAX(MIN(ptStart.x, ptEnd.x), 0),lWidth-1);
		lEnd = MIN(MAX(MAX(ptStart.x, ptEnd.x), 0),lWidth-1);
		for (x=lStart; x<=lEnd;x++)
		{
				...
				MPOINT ptStart, MPOINT ptEnd, MDouble dCoeffK, MDouble dCoeffB, MByte bVertical)
{
	...
	if (bVertical)//是垂直的line
	{
		 ...
		 //make sure the minValue/maxValue in image size
		 minValue = MAX(minValue, 0);
		 maxValue = MIN(maxValue, lHeight-1);
	...
	if (dCoeffK<1.0 && dCoeffK>-1.0)
	{
		...
		minValue = MAX(minValue, 0);
		maxValue = MIN(maxValue, lWidth-1);
...
MVoid vDrawLine3(MByte* pRltData, MLong lRltLine, MLong lWidth, MLong lHeight, 
							MByte destFlag, MDouble dCoeffK, MDouble dCoeffB, MBool bVert)
{
	MLong x, y;
	if (bVert)
	{
	...
	else if (dCoeffK<1.0 && dCoeffK>-1.0)
	{
		for (x = 0; x<lWidth  ; x++)
		{
			y = (MLong)(x * dCoeffK + dCoeffB);
			if (y<0 || y>=lHeight)continue;
			*(pRltData + lRltLine * y + x) = destFlag;
		}
	}
...
MDouble vPointDistToLine2D(MPOINT pt, MDouble dCoeffK, MDouble dCoeffB, MLong bVert)	//xsd
{
	...
	if (bVert)
	{
		tmp = pt.x + dCoeffB;	//这里要注意，我们采用的是0 = -x+b的方程
	}
	else
	{
		tmp = (pt.x*dCoeffK+dCoeffB-pt.y)/sqrt(dCoeffK*dCoeffK+1);
	}
	res = fabs(tmp);
	...
	MPOINT *pCrossPT1, MPOINT* pCrossPT2)
{
	...
	if (bVert)
	{
		x1 = x2 = (MLong)(-CoeffB);
		lDeltaX = xc - x1;
		y1 = (MLong)(yc + sqrt(r*r - lDeltaX * lDeltaX));
		y2 = (MLong)(yc - sqrt(r*r - lDeltaX * lDeltaX));
	}
	else
	{
