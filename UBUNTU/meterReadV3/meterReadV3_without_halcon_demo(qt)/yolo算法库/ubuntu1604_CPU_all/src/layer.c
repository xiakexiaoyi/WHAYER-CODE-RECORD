...
void free_layer(layer l)
{
	...
	//if (l.input_layers)      free(l.input_layers);
	//if (l.input_sizes)       free(l.input_sizes);
	if (l.input_layer)       free(l.input_layer);
	if (l.self_layer)        free(l.self_layer);
	if (l.output_layer)      free(l.output_layer);

	if (l.input_gate_layer)  free(l.input_gate_layer);
	if (l.state_gate_layer)  free(l.state_gate_layer);
	if (l.input_save_layer)  free(l.input_save_layer);
	if (l.state_save_layer)  free(l.state_save_layer);
	if (l.input_state_layer) free(l.input_state_layer);
	if (l.state_state_layer) free(l.state_state_layer);

	if (l.input_z_layer)    free(l.input_z_layer);
	if (l.state_z_layer)    free(l.state_z_layer);

	if (l.input_r_layer)    free(l.input_r_layer);
	if (l.state_r_layer)    free(l.state_r_layer);

	if (l.input_h_layer)    free(l.input_h_layer);
	if (l.state_h_layer)    free(l.state_h_layer);
	if (l.softmax_tree)       free(l.softmax_tree);
