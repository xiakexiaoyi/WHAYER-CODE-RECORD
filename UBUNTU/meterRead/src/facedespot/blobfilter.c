...
//lRadius是用于计算局部区域的半径
MRESULT LocalMax_Circle1(MHandle hMemMgr, MByte* pSrc, MLong lSrcLine, 
						  MByte* pRlt, MLong lRltLine,
						  MLong lWidth, MLong lHeight, 
						  MVoid *pTmpMem, MLong lMemLen, 
						  MLong lHighThres, MLong lLowThres, MLong lBaseLowRate,
						  MByte rltLabel, JGSEED *pSeeds, MLong lRadius)
{
	...
	for(j=lRadius; j<lHeight-lRadius; j++, pSrcData+=lSrcExt,pRltData+=lRltExt)
	{
		for (i=lRadius; i<lWidth-lRadius; i++, pSrcData++,pRltData++)
		{
			if (*pRltData == rltLabel)  //rltLabel是怎么标的 在softThres里
			{
				lMaxRes = 0;
				//求局部最大
				for (k =-lRadius; k<lRadius; k++)
				{
					for (l=-lRadius; l<lRadius; l++)
					{
						if (/**pSrcData<*(pSrcData+k*lSrcLine+l)
							|| */*(pRltData+k*lRltLine+l)==250)
							break;
						if(*(pSrcData+k*lSrcLine+l)>lMaxRes)
						{
							lMaxRes = *(pSrcData+k*lSrcLine+l);
							lMaxIndex.x = l;
							lMaxIndex.y = k;
				...
				if (k==lRadius&& l==lRadius)
				{//A seed, enqueue
					pSeeds->pptSeed[pSeeds->lSeedNum].x = i+ lMaxIndex.x;
					pSeeds->pptSeed[pSeeds->lSeedNum].y = j+ lMaxIndex.y;
					pSeeds->pcrSeed[pSeeds->lSeedNum] = *(pSrcData+lMaxIndex.y*lSrcLine+lMaxIndex.x) ;
					pSeeds->lSeedNum++;
