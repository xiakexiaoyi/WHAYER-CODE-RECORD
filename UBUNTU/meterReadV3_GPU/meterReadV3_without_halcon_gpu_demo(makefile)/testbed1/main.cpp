...
//match
    ...
    tmpImage = cvCloneImage(&imgTmp);
    cvSaveImage("testImage.bmp", tmpImage);
    wy_testImage.lWidth = tmpImage->width;
    wy_testImage.lHeight = tmpImage->height;
    wy_testImage.pixelArray.chunky.lLineBytes = tmpImage->widthStep;
    wy_testImage.pixelArray.chunky.pPixel = tmpImage->imageData;
    ...
    while (1)
    {
        ...
        if (mouseParam[2]==CV_EVENT_LBUTTONDOWN)
        {
            ...
            flagL=1;
            cvShowImage(WINDOW_NAME,tmpImage);
            left = startPt.x;
            top = startPt.y;
        }
    ...
    cv::destroyWindow(WINDOW_NAME);
    origin_x=(left+right)/2;//计算画框中心点
    origin_y=(top+bottom)/2;

    ImgSize.height = tmpImage->height;
    ImgSize.width = tmpImage->width;
...
int detect(const char *input, MV3Para *pOutPattern)
{
    ...
    if (0 != yoloinit_GPU(&handle, cfgfile, weightfile, 0.24,0))//初始化
    {
    ...
    //while(1)
    {
        ...
        if (0 != HYL_GetTemplateFromText(MHandle, MR_READ_FILE_PARA))//读取模板
        {
            printf("HYAMR_GetTemplateFromText error !\n");
        }
        imgs.lWidth = src->width;
        imgs.lHeight = src->height;
        imgs.pixelArray.chunky.lLineBytes = src->widthStep;
        imgs.pixelArray.chunky.pPixel = src->imageData;
        imgs.lPixelArrayFormat = HYAMR_IMAGE_BGR;
        HYL_GetDashboard(MHandle, &imgs, "OK", 0.45, offset);//匹配模板,求图像偏移量
        int offset_x=offset->x-origin_x;
        int offset_y=offset->y-origin_y;
        printf("offset_x=%d offset_y=%d\n",offset_x,offset_y);
        pOutPattern->Col+=offset_x;
        pOutPattern->Row+=offset_y;
        for(int i=0;i<pOutPattern->lPtNum;i++)
        {
            pOutPattern->ptPosList[i].x+=offset_x;
            pOutPattern->ptPosList[i].y+=offset_y;
        }
        ...
        if(0!=MeterReadRecogV3_GPU(handle,&image, *pOutPattern,MeterResult))//检测
        {
