...
void train_char_rnn(char *cfgfile, char *weightfile, char *filename, int clear, int tokenized)
{
    ...
    inputs = get_network_input_size(net);
    fprintf(stderr, "Learning Rate: %g, Momentum: %g, Decay: %g\n", net.learning_rate, net.momentum, net.decay);
     batch = net.batch;
     steps = net.time_steps;
    if(clear) *net.seen = 0;
     i = (*net.seen)/net.batch;

    streams = batch/steps;
    offsets = calloc(streams, sizeof(size_t));
    ...
    while(get_current_batch(net) < net.max_batches){
        ...
        loss = train_network_datum(net, p.x, p.y) / (batch);
        free(p.x);
        free(p.y);
        if (avg_loss < 0) avg_loss = loss;
        avg_loss = avg_loss*.9 + loss*.1;

        chars = get_current_batch(net)*batch;
        fprintf(stderr, "%d: %f, %f avg, %f rate, %lf seconds, %f epochs\n", i, loss, avg_loss, get_current_rate(net), sec(clock()-time1), (float) chars/size);
...
void run_char_rnn(int argc, char **argv)
{
    ...
    filename = find_char_arg(argc, argv, "-file", "data/shakespeare.txt");
    seed = find_char_arg(argc, argv, "-seed", "\n\n");
    len = find_int_arg(argc, argv, "-len", 1000);
    temp = find_float_arg(argc, argv, "-temp", .7);
    rseed = find_int_arg(argc, argv, "-srand", time(0));
    clear = find_arg(argc, argv, "-clear");
    tokenized = find_arg(argc, argv, "-tokenized");
    tokens = find_char_arg(argc, argv, "-tokens", 0);
