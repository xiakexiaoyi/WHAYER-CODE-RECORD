...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net.max_batches){
        ...
        printf("%d: %f, %f avg, %f rate, %lf seconds, %d images\n", get_current_batch(net), loss, avg_loss, get_current_rate(net), sec(clock()-time1), i*imgs);
...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile)
{
    ...
    type = option_find_str(options, "eval", "voc");
    ...
    m = plist->size;
    val = calloc(nthreads, sizeof(image));
    val_resized = calloc(nthreads, sizeof(image));
    buf = calloc(nthreads, sizeof(image));
    buf_resized = calloc(nthreads, sizeof(image));
    thr = calloc(nthreads, sizeof(pthread_t));
...
void validate_detector_recall(char *datacfg, char *cfgfile, char *weightfile)
{
	...
	valid_images = option_find_str(options, "valid", "data/train.txt");
    plist = get_paths(valid_images);
    paths = (char **)list_to_array(plist);
...
int  COM_YoloDetector_GPU(const network net, const image pImg, float thresh, int *numDST, float **boxout)
{
	...
	if(nms)
	{
		if(0!=do_nms_sortR(boxes, probs, l.w*l.h*l.n, l.classes, nms))
		{
			res=-1;
			goto EXT;
		}
	}
	box_detections(im, l.w*l.h*l.n, thresh, boxes, probs, l.classes, numDST, boxout);
