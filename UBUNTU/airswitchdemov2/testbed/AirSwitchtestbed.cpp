...
int AirSwitchRecog(HYAR_RESULT_LIST resultlist)
{
	...
	char *weightfile="../../model/tiny-yolo-voc_21000.weights";// 模型 
	//char *filename="../223.jpg";
    MPOINT *centre;
	...
	HYAR_Init(NULL,&hTLHandle);//
	HYAR_SetParam(hTLHandle,cfgfile,weightfile,thresh,w,h);//
	...
	//for(int i=0;i<resultlist.lAreaNum;i++)//实际没有循环
	{
		...
		HYAR_AirSwitchRecog(hTLHandle,&imgs,&resultlist);//
