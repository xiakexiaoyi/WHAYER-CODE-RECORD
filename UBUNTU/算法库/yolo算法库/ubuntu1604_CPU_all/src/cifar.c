...
void test_cifar_multi(char *filename, char *weightfile)
{
    ...
    for(i = 0; i < test.X.rows; ++i){
		float pred[10] = {0};
        im = float_to_image(32, 32, 3, test.X.vals[i]);
        ...
        index = max_index(pred, 10);
        class = max_index(test.y.vals[i], 10);
...
void test_cifar(char *filename, char *weightfile)
{
    ...
    avg_acc += acc[0];
    avg_top5 += acc[1];
    printf("top1: %f, %lf seconds, %d images\n", avg_acc, sec(clock()-time1), test.X.rows);
    free_data(test);
}
...
void extract_cifar()
{
    ...
    for(i = 0; i < train.X.rows; ++i){
        ...
        class = max_index(train.y.vals[i], 10);
