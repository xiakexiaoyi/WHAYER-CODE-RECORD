...
box_label *read_boxes(char *filename, int *n)
{
	...
	box_label *boxes;
	FILE *file;
    boxes =(box_label*)calloc(1, sizeof(box_label));
    file = fopen(filename, "r");
    if(!file) file_error(filename);
...
{
	int i,k,oh,ow,dh,dw;
	int pleft,pright,ptop,pbot,swidth,sheight,flip;
	float sx,sy,dx,dy;
	char **random_paths;
	image orig,cropped,sized;
	data d = {0};
    ...
    for(i = 0; i < n; ++i){
        ...
        pleft  = rand_uniform_strong(-dw, dw);
        pright = rand_uniform_strong(-dw, dw);
        ptop   = rand_uniform_strong(-dh, dh);
        pbot   = rand_uniform_strong(-dh, dh);
...
{
	int k,index,dw,dh,h,w;
	int pleft,pright,ptop,pbot;
	int swidth,sheight,flip;
	float sx,sy,dx,dy;
	image cropped, sized,orig;
	char *random_path;
...
pthread_t load_data_in_thread(load_args args)
{
    ...
    ptr = (struct load_args*)calloc(1, sizeof(struct load_args));
    *ptr = args;
...
void *load_threads(void *ptr)
{
    ...
    total = args.n;
    free(ptr);
    buffers = (data *)calloc(args.threads, sizeof(data));
...
data *split_data(data d, int part, int total)
{
	...
	int i,start,end;
    split = (data *)calloc(2, sizeof(data));
   
    start = part*d.X.rows/total;
    end = (part+1)*d.X.rows/total;
