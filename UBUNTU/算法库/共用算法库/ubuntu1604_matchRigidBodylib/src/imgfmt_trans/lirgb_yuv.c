...
#ifdef OIL_AMMETER_COLOR_CONVERT

#include <math.h>
//#include <stdio.h>
/*MDouble SigmoidFunc(MDouble x, MDouble Scale)
{
	if (Scale < 0.0001 && Scale >-0.0001)return -1;
	x = x/Scale;
	x = 1/(exp(x)+1)+0.5;
	return x;
}*/
...
MLong SCALE_SUPPORT[] = {20};
MLong SigmoidValue[256] = 
{256, 252, 249, 246, 243, 240, 236, 233, 230, 227, 224, 221, 218, 215, 212, 210, 207, 204, 201, 199, 196, 194, 191, 189, \
187, 185, 182, 180, 178, 176, 174, 172, 171, 169, 167, 165, 164, 162, 161, 159, 158, 157, 155, 154, 153, 152, 151, 150, \
149, 148, 147, 146, 145, 144, 144, 143, 142, 141, 141, 140, 140, 139, 139, 138, 138, 137, 137, 136, 136, 135, 135, 135, \
134, 134, 134, 133, 133, 133, 133, 132, 132, 132, 132, 131, 131, 131, 131, 131, 131, 130, 130, 130, 130, 130, 130, 130, \
130, 129, 129, 129, 129, 129, 129, 129, 129, 129, 129, 129, 129, 129, 129, 128, 128, 128, 128, 128, 128, 128, 128, 128, \
...
//根据油位表的特点，为了突出显示红色标记的刻度，BGR转Y的方法要略作调整，根据，BG分量来求Y,再根据R分量调整Y,R分量越大，Y越小
...
/*			RGB2Y(IMG_BGR[2], IMG_BGR[1], IMG_BGR[0], IMG_Y);//20140812@bqj 好像是B分量和R分量反掉了
			if (*IMG_Y<IMG_BGR[2])
			{
				//*IMG_Y = *IMG_Y*SigmoidFunc(IMG_BGR[2]-*IMG_Y, 20);
				*IMG_Y = *IMG_Y*SigmoidValue[IMG_BGR[2]-*IMG_Y]/256;
			}		*/
...
{
	MDWord x,y;
	MDWord dwRGBExt = dwRGBLine - 3*dwWidth; 
	MDWord dwYExt = dwYLine - dwWidth;
	for(y=dwHeight; y!=0; y--, IMG_RGB+=dwRGBExt, IMG_Y+=dwYExt)
	{
		for(x=dwWidth; x!=0; x--, IMG_RGB+=3, IMG_Y++)
		{
			RGB2Y(IMG_RGB[0], IMG_RGB[1], IMG_RGB[2], IMG_Y);
		}
	}
}
