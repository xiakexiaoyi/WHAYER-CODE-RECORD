					  ...
					  MLong* plHighThres)
{
	...
	MDWord factor=((HIST_LENGTH)<<16)/dwMaxGrad;
    ...
    MDWord* pMaxGrad, MDWord *plNum)
{
    ...
    //MUInt16 *tmpMag,*tmpMagTail;
    MByte* tmpMask;
    MLong lSrcExt, lMaskExt, lGradExt;
    MLong nw, n, ne, w, c, e, sw, s, se;
    MDWord dwSum=0;
    MDWord MaxGrad=0;
    MDWord dwNum=0;

    pTmpSrc = (MByte*)pSrcData+lSrcLine+1;
    tmpMask = pMaskData + lMaskLine+ 1;
    tmpGx=Gx+lGraLine+1;
    tmpGy=Gy+lGraLine+1;
    lSrcExt = lSrcLine - (lWidth-1-1);
    lGradExt = lGraLine - (lWidth-1-1);
    lMaskExt = lMaskLine - (lWidth-1-1);
    for (j=1; j<lHeight-1; j++,pTmpSrc+=lSrcExt,tmpGx+=lGradExt, tmpGy+=lGradExt, tmpMask+=lMaskExt)
    {
        // 		nw = *(pTmpSrc-lSrcLine-1);
        // 		n = *(pTmpSrc-lSrcLine);
        // 		w = *(pTmpSrc-1);
            ...
            tmpGx++, tmpGy++, tmpMask++)
        {
            ...
            gxVal = (MShort)(((e - w)*2)+(ne-nw)+(se-sw));
            gyVal = (MShort)(((s-n)*2)+(se-ne)+(sw-nw));
            gSum = ABS(gxVal)+ABS(gyVal);
            //nw=n;w=c;sw=s;n=ne;c=e;s=se;
