...
static MVoid GetResponseResult(JIntegral *pIntegral, MLong lIntegralLine,MLong *pDstData, 
								MLong lDstLine,MLong lWidth, MLong lHeight,PHaarParam pParam, MLong lMeterType);
...
static MRESULT CalcLineEndPt_xsd(MHandle hMemMgr, BLOCK *pHaarResponse, MLong lRadius, MLong lThreshold, LINEPARAM *lineParam, MLong lMeterType);
...
//求刻度位置的时候，使用了Haar 算子求响应图
//部分参数是人工设置的，后期需要根据实际情况做自适应
//有:
//1   haar模板的宽度(pixels)
//2 Haar响应图求联通区域时需要设定阈值(120)
//3 最小联通区域面积(10)
//4 RANSAC时求是否在圆上的半径误差(5), PS:这个应该跟Haar模板的宽度是关联的
MRESULT GetPixelLocation(MHandle hMemMgr,BLOCK *pImage, PARAM_INFO *pParam, MLong lMeterType)
{
	...
	case OIL_METER:
		{
			lHaarWidth=DIV_ROUND(pImage->lHeight,50);
			JPrintf("lHaarWidth=%d\n", lHaarWidth);
			haarParam.h = 3*lHaarWidth;
			haarParam.w = lHaarWidth<<2;
			break;
		}
	case OIL_TEMPERATURE:
		{
			...
			JPrintf("lHaarWidth=%d\n", lHaarWidth);
			haarParam.h = 3*lHaarWidth;
			haarParam.w = lHaarWidth<<1;
			break;
		}
	case VOLT_METER:
		{
			lHaarWidth = DIV_ROUND(pImage->lHeight, 110);
			JPrintf("volt   lHaarWidth=%d\n", lHaarWidth);
	...
	case SF6_GAS_METER:
		{
			lHaarWidth = DIV_ROUND(pImage->lHeight, 70);
			//lHaarWidth = 4;
			JPrintf("SF6   lHaarWidth=%d\n", lHaarWidth);
			haarParam.h = lHaarWidth * 3;
			haarParam.w = lHaarWidth * 6;
			break;
		}
	case DISCHARGE_METER:
		{
			lHaarWidth = DIV_ROUND(pImage->lHeight, 70);
			//lHaarWidth = 4;
			JPrintf("Discharge   lHaarWidth=%d\n", lHaarWidth);
			haarParam.h = lHaarWidth * 3;
			haarParam.w = lHaarWidth * 6;
			break;
		}
	case SWITCH_METER:
		{
			lHaarWidth = DIV_ROUND(pImage->lHeight, 35);
			//lHaarWidth = 4;
			JPrintf("Discharge   lHaarWidth=%d\n", lHaarWidth);
			haarParam.h = lHaarWidth * 3;
			haarParam.w = lHaarWidth * 6;
...
//	printf("stride=%d, width=%d, height=%d\n", pImage->lBlockLine, pImage->lWidth, pImage->lHeight);
	GO(HaarResponseAngle(hMemMgr,pImage,&haarParam,&haarResult, &haarAngleResult, lMeterType));
	...
	GO(StatisticNonZeroValue(hMemMgr, (MByte*)haarResult.pBlockData, haarResult.lBlockLine,haarResult.lWidth,
								haarResult.lHeight,0, pTemp, sizeof(MPOINT)*lStackLen, 30, &lThreshold));
	JPrintf("threshold=%d\n", lThreshold);
	//这里的阈值需要根据实际情况选择合适的
	GO(LocalMax_Circle(hMemMgr, (MByte*)haarResult.pBlockData, haarResult.lBlockLine, (MByte *)blockFlag.pBlockData,blockFlag.lBlockLine,\
		blockFlag.lWidth,blockFlag.lHeight,pTemp,lStackLen, lThreshold, (MLong)(lThreshold*0.6), 50, 128, &seedsTemp, lHaarWidth));

	PrintBmpEx(blockFlag.pBlockData, blockFlag.lBlockLine, DATA_U8, blockFlag.lWidth, blockFlag.lHeight, 1,"D:\\localMaxPre_Circle_aaa.bmp");

	AllocVectMem(hMemMgr,pSeedsAngle, seedsTemp.lSeedNum, MLong);
	SetVectZero(pSeedsAngle,seedsTemp.lSeedNum * sizeof(MLong));
	GO(CalcCircleCenter(hMemMgr, &haarAngleResult, &seedsTemp, pSeedsAngle, &ptCenter, lHaarWidth<<2));
	GO(CalcCircleRadius(hMemMgr, &haarResult, &seedsTemp, pSeedsAngle, ptCenter, &lRadius));
	vDrawCircle((MByte*)blockFlag.pBlockData, blockFlag.lBlockLine, blockFlag.lWidth, blockFlag.lHeight, ptCenter.x, ptCenter.y, 4, 255);
	vDrawCircle((MByte*)blockFlag.pBlockData, blockFlag.lBlockLine, blockFlag.lWidth, blockFlag.lHeight, ptCenter.x, ptCenter.y, lRadius, 255);
	vDrawCircle((MByte*)blockFlag.pBlockData, blockFlag.lBlockLine, blockFlag.lWidth, blockFlag.lHeight, ptCenter.x, ptCenter.y, lRadius-25, 255);
	PrintBmpEx(blockFlag.pBlockData, blockFlag.lBlockLine, DATA_U8, blockFlag.lWidth, blockFlag.lHeight, 1,"D:\\Circle.bmp");
...
//******************************** line location*************************************************
MRESULT GetLineLocation2(MHandle hMemMgr,BLOCK *pImage, PARAM_INFO *pParam, MLong lMeterType)
{
	MRESULT res=LI_ERR_NONE;
	JGSEED seedsTemp={0};
	MLong i;
	...
	MLong lStackLen;
	HaarParam haarParam={0};
	BLOCK blockFlag={0};//算法定型的时候这块就去掉
	MLong lSeedSize;
	MLong lHaarWidth=0;
	...
	CIRCLE circleParam = {0};
	ELLIPCE ellipce = {0};
	LINEPARAM *pLineParam = MNull;

	haarParam.lClassNumber=4;
	haarParam.lAngleNumber=180/ANGLE_STEP;
	haarParam.lAngleNumber=1;

	switch(lMeterType)
	{
	case OIL_METER:
		{
			lHaarWidth=DIV_ROUND(pImage->lHeight,45);
			JPrintf("oil    lHaarWidth=%d\n", lHaarWidth);
			haarParam.h = 3*lHaarWidth;
			haarParam.w = lHaarWidth<<2;
			break;
		}
	case OIL_TEMPERATURE:
		{
			lHaarWidth=DIV_ROUND(pImage->lHeight,50);	// 60
			JPrintf("temp   lHaarWidth=%d\n", lHaarWidth);
			haarParam.h=3*lHaarWidth;			
			haarParam.w=(lHaarWidth<<2);
			break;
		}
	...
	case SF6_GAS_METER:
		{
			...
			fprintf(tmpFp, "2015-05-25 18:58\n");
			fprintf(tmpFp, "SF6   lHaarWidth=%d\n", lHaarWidth);
			fclose(tmpFp);*/
	...
	case QUALITROL_METER:
		{
			lHaarWidth=DIV_ROUND(pImage->lHeight,28);
			printf("qualitrol lHaarWidth=%d\n", lHaarWidth);
			haarParam.h = 3*lHaarWidth;
			haarParam.w = lHaarWidth<<2;
			break;
		}
	case LEAKAGE_CURRENT_METER:
		{
			lHaarWidth = 5;
			printf("Leakage lHaarWidth=%d\n", lHaarWidth);
			haarParam.h = lHaarWidth * 3;
			haarParam.w = lHaarWidth<<2;
			break;
		}
	...
	{
		BLOCK edgeImg = {0};
		GO(B_Create(hMemMgr, &edgeImg, DATA_U8, pImage->lWidth, pImage->lHeight));
		GO(Edge(hMemMgr, pImage->pBlockData, pImage->lBlockLine,
				pImage->lWidth, pImage->lHeight, (MByte*)(edgeImg.pBlockData), edgeImg.lBlockLine, TYPE_CANNY));
		PrintBmpEx(edgeImg.pBlockData, edgeImg.lBlockLine, DATA_U8, edgeImg.lWidth, edgeImg.lHeight, 1, "D:\\edge.bmp");
		B_Release(hMemMgr, &edgeImg);
	}
		
	AllocVectMem(hMemMgr,haarParam.pAngleList,haarParam.lAngleNumber,MLong);
	for (i=0;i<haarParam.lAngleNumber;i++)    //逆时针旋转
	{
		haarParam.pAngleList[i] = i*ANGLE_STEP;//横向的模板,最后要-90度
		
	}
	haarParam.pAngleList[0] = 118;  //43
	/*haarParam.pAngleList[1] = 45;
	haarParam.pAngleList[2] = 59;
	haarParam.pAngleList[3] = 73;
	haarParam.pAngleList[4] = 99;
	haarParam.pAngleList[5] = 110;
	haarParam.pAngleList[6] = 118;*/
	GO(B_Create(hMemMgr, &haarAngleResult, DATA_U8, pImage->lWidth, pImage->lHeight));
	GO(B_Create(hMemMgr,&haarResult,DATA_U8,pImage->lWidth, pImage->lHeight));
    //获取响应图
    PrintBmpEx(pImage->pBlockData, pImage->lBlockLine, DATA_U8, pImage->lWidth, pImage->lHeight, 1, "D:\\gray_bbb.bmp");

	if (LEAKAGE_CURRENT_METER == lMeterType)
	{
		GO(GetCircleRing(pImage, &pParam->pCircleInfo->circleParam, lHaarWidth, LEAKAGE_CURRENT_METER));
		PrintBmpEx(pImage->pBlockData, pImage->lBlockLine, DATA_U8, pImage->lWidth, pImage->lHeight, 1, "D:\\srcRing.bmp");
	}
//	GO(HaarResponseAngle(hMemMgr,pImage,&haarParam,&haarResult, &haarAngleResult));
	HaarResponseAngle(hMemMgr,pImage,&haarParam,&haarResult, &haarAngleResult, lMeterType);
	sprintf(textArr, "D:\\response_bbb.bmp");
	PrintBmpEx(haarResult.pBlockData, haarResult.lBlockLine, DATA_U8, haarResult.lWidth, haarResult.lHeight, 1, textArr);
	Logger("haar response ok...\n");
	...
	if (lMeterType == SF6_GAS_METER)
	{
		JPrintf("circleInfo: (%d,%d) %d\n", pParam->pCircleInfo->circleParam.xcoord, pParam->pCircleInfo->circleParam.ycoord, pParam->pCircleInfo->circleParam.lRadius);
		GO(GetCircleRing(&haarResult, &pParam->pCircleInfo->circleParam, lHaarWidth, SF6_GAS_METER));
		sprintf(textArr, "D:\\ring.bmp");
		PrintBmpEx(haarResult.pBlockData, haarResult.lBlockLine, DATA_U8, haarResult.lWidth, haarResult.lHeight, 1, textArr);
	}

	GO(StatisticNonZeroValue(hMemMgr, (MByte*)haarResult.pBlockData, haarResult.lBlockLine,haarResult.lWidth,
		haarResult.lHeight,0, pTemp, sizeof(MPOINT)*lStackLen, 35, &lThreshold));	//90%	75
	JPrintf("threshold=%d\n", lThreshold);
	//这里的阈值需要根据实际情况选择合适的
	GO(LocalMax_Circle(hMemMgr, (MByte*)haarResult.pBlockData, haarResult.lBlockLine, (MByte *)blockFlag.pBlockData,blockFlag.lBlockLine,\
		blockFlag.lWidth,blockFlag.lHeight,pTemp,lStackLen, lThreshold, (MLong)(lThreshold*0.6), 50, 128, &seedsTemp, lHaarWidth>>1));
	...
	if (lMeterType == SF6_GAS_METER)
	{
		ridgePoint(hMemMgr, (MByte*)haarResult.pBlockData, haarResult.lBlockLine, haarResult.lWidth, haarResult.lHeight, 
			(MByte*)blockFlag.pBlockData, blockFlag.lBlockLine, lHaarWidth>>1, (MLong)(lThreshold*0.6), 0, 250, &seedsTemp);

		PrintBmpEx(blockFlag.pBlockData, blockFlag.lBlockLine, DATA_U8, blockFlag.lWidth, blockFlag.lHeight, 1,"D:\\localMaxPost.bmp");
		JPrintf("ridgePoint--------\n");

		Logger("sf6 ridge point ok...\n");
		...
		/*JPrintf("seedNum = %d\n", seedsTemp.lSeedNum);
		tmpFp = fopen("D:\\log.dat", "ab+");
		fprintf(tmpFp, "seedNum = %d\n", seedsTemp.lSeedNum);
		fclose(tmpFp);*/
		GO(RANSAC_LINE_SF6(hMemMgr, &haarResult, &haarAngleResult, seedsTemp.pptSeed, seedsTemp.lSeedNum,pTemp, lStackLen, pParam, lHaarWidth*10));
		Logger("sf6 ransac line ok...\n");
	}
	else if (lMeterType == SWITCH_METER)
	{
		ridgePoint(hMemMgr, (MByte*)haarResult.pBlockData, haarResult.lBlockLine, haarResult.lWidth, haarResult.lHeight, 
			(MByte*)blockFlag.pBlockData, blockFlag.lBlockLine, lHaarWidth>>2, (MLong)(lThreshold*0.6), 0, 250, &seedsTemp);

		PrintBmpEx(blockFlag.pBlockData, blockFlag.lBlockLine, DATA_U8, blockFlag.lWidth, blockFlag.lHeight, 1,"D:\\localMaxPost.bmp");
		JPrintf("ridgePoint--------\n");

		Logger("switch ridge point ok...\n");
		...
		GO(RANSAC_LINE_SWITCH(hMemMgr, &haarResult, &haarAngleResult, seedsTemp.pptSeed, seedsTemp.lSeedNum,pTemp, lStackLen, pParam));
		Logger("swithc ransac line ok...\n");
		goto EXT;
	}
	else
	{
		...
		Logger("meters ransac line ok...\n");
	}
	JPrintf("ransac line ok!\n");

	pLineParam = &(pParam->pCircleInfo->lineInfo.lineParam);
	lThreshold = lThreshold * 3 / 5;
	CalcLineEndPt_xsd(hMemMgr, &haarResult, pParam->pCircleInfo->circleParam.lRadius, 50, pLineParam, lMeterType);
EXT:
	B_Release(hMemMgr,&haarResult);
	B_Release(hMemMgr, &haarAngleResult);
	FreeVectMem(hMemMgr,haarParam.pAngleList);
...
MRESULT HaarResponseAngle(MHandle hMemMgr ,BLOCK *pImage, PHaarParam pParam,BLOCK *pResponseResultImage,BLOCK *pResponseAngle, MLong lMeterType)
{
	MRESULT res=LI_ERR_NONE;
	MLong i, j;
	BLOCK imageRotate1={0};
	MLong lRotateWidth;
	MLong lRotateHeight;
	BLOCK xDstBlock={0};
	BLOCK yDstBlock={0};
	...
	GO(B_Create(hMemMgr,&imageRotate1,DATA_U8,lRotateWidth,lRotateHeight));
	...
	AllocVectMem(hMemMgr,responseImage.pData,imageRotate1.lBlockLine*lRotateHeight,MLong);
	SetVectZero(responseImage.pData, imageRotate1.lBlockLine*lRotateHeight*sizeof(MLong));
	...
	{
      //旋转图像，这段程序需要优化一下
		RotateImage_xsd(pImage,&imageRotate1,pParam->pAngleList[i],&xDstBlock,&yDstBlock);
		responseImage.lBlockLine=imageRotate1.lBlockLine;
		responseImage.lWidth=imageRotate1.lWidth;
		responseImage.lHeight=imageRotate1.lHeight;
        //获取haar响应图
		lIntegralLine = CEIL_4(imageRotate1.lWidth);
		Integral(imageRotate1.pBlockData, imageRotate1.lBlockLine, DATA_U8,pIntegral,MNull, lIntegralLine,imageRotate1.lWidth,imageRotate1.lHeight);
		GetResponseResult(pIntegral, lIntegralLine,responseImage.pData,responseImage.lBlockLine,
							imageRotate1.lWidth, imageRotate1.lHeight,pParam, lMeterType);
...
EXT:
	...
	B_Release(hMemMgr,&imageRotate1);
...
MVoid GetResponseResult(JIntegral *pIntegral, MLong lIntegralLine,
						...
						PHaarParam pParam, MLong lMeterType)
{
	MLong i,j;
	MUInt32 lTrimValue;
...
//		factor = 16;
		...
		for (j=h;j<lHeight;j++,pDstData+=lDstExt+w, pIntegral+=lIntExt+w)
		{
			...
			for (i=w;i<lWidth;i++,pDstData++, pIntegral++)
			{
				...
				if (sum3<(sum2)&&sum3<(sum4))          //白线黑线
					lTrimValue =(sum2+sum4)/2-sum3;
				//if (sum3>(sum2)&&sum3>(sum4))
				//lTrimValue = sum3 - (sum2+sum4)/2; 
				//if(OIL_METER == lMeterType || QUALITROL_METER==lMeterType)
				//{
				//	if (sum3<(sum2*factor>>4)&&sum3<(sum4*factor>>4))	//表盘背景亮度高于指针亮度
				//		lTrimValue =(sum2+sum4)/2-sum3;
				//	else if(sum2<(sum3*factor>>4)&&sum4<(sum3*factor>>4))	//表盘背景为黑色 指针为白色
				//		lTrimValue = sum3 - (sum2+sum4)/2; 
				//}
				//else if (LEAKAGE_CURRENT_METER == lMeterType)
				//{
				//	if(sum3<((MUInt32)(sum2*0.95)) && sum3<((MUInt32)(sum4*0.95)))
				//		lTrimValue =(sum2+sum4)/2-sum3;
				//	else
				//		lTrimValue = 0;
				//}
...
//	JPrintf("CreateLineSet~~~\n");
	{
		...
		maxDist2Center = pCircleInfo->circleParam.lRadius>>1;	//1/4 ?  1/2 ?
		randomPoint[2].x = pCircleInfo->circleParam.xcoord;
		randomPoint[2].y = pCircleInfo->circleParam.ycoord;
			...
			for (k=j+1; k<seedTmp.lSeedNum; k++)
			{
				...
				while(bChanged)
				{
					...
					for (m = 0 ; m<seedTmp.lSeedNum; m++)
					{
						...
						dTmpAngle=ABS(lAngleValue-lAngleValue2)*ANGLE_STEP;
						if (dTmpAngle>10 &&dTmpAngle<170)continue;
						if (distance < RANSAC_THRESHOLD3/2)
						{
							pTmpPt[numInPoint].x = seedTmp.pptSeed[m].x;
							pTmpPt[numInPoint++].y = seedTmp.pptSeed[m].y;
						}
						...
						MLong lPtNum, MPOINT *pTemp,MLong lTmpNum, PARAM_INFO *pParam, MLong lMaxPtDist)
{
	...
	//	JPrintf("CreateLineSet~~~\n");
	{
		...
		for (j=0; j<lineSetInfo.lLineNum; j++)
		{
			...
			lTmpPtDist = vDistance3L(pLineInfo->pPtinfo[0].ptPoint, pLineInfo->pPtinfo[pLineInfo->lPtNum-1].ptPoint);
			if (lTmpPtDist > lMaxPtDist)
			{
				pLineInfo->lConfidence = 0;
				continue;
			}
		...
		//直线与圆的交点
		{
			...
			{	
				...
				sprintf(path, "D:\\line.bmp");
				B_Cpy(&blockFlagTmp1, pHaarResponse);
				vDrawLine2(blockFlagTmp1.pBlockData, blockFlagTmp1.lBlockLine, blockFlagTmp1.lWidth, blockFlagTmp1.lHeight, 
					255, pLineInfo->lineParam.Coefk, pLineInfo->lineParam.Coefb);
				PrintBmpEx(blockFlagTmp1.pBlockData, blockFlagTmp1.lBlockLine, DATA_U8, blockFlagTmp1.lWidth, blockFlagTmp1.lHeight, 1,path);
			}
		}
...
MRESULT Get_SF6_Line(MHandle hMemMgr, BLOCK *pHaarResponse, CIRCLE circleParam, PARAM_INFO *pParam)
{
	...
	{	
		...
		B_Cpy(&blockFlagTmp1, pHaarResponse);
		vDrawLine2((MByte*)blockFlagTmp1.pBlockData, blockFlagTmp1.lBlockLine, blockFlagTmp1.lWidth, blockFlagTmp1.lHeight, 
			255, maxLine.lineParam.Coefk, maxLine.lineParam.Coefb);
...
MRESULT CalcLineEndPt_xsd(MHandle hMemMgr, BLOCK *pHaarResponse, MLong lRadius, MLong lThreshold, LINEPARAM *lineParam, MLong lMeterType)
{
	MRESULT res = LI_ERR_NONE;
	MLong i;
	...
	if (ptLeft.x==ptRight.x)
	{
		leftVal = pData + (ptMid.y-1) * lStride + ptMid.x;
		rightVal = pData + (ptMid.y+1) * lStride + ptMid.x;
		for (i=0; i<lMaxLength; i++, leftVal-=lStride, rightVal+=lStride)	
		{
	...
	{
		leftVal = pData + ptMid.y * lStride + ptMid.x - 1;
		rightVal = pData + ptMid.y * lStride + ptMid.x + 1;
		for (i=0; i<lMaxLength; i++, leftVal--, rightVal++)	
		{
			if (*leftVal >= lThreshold)
				leftLength++;
			if(*rightVal >= lThreshold)
	...
	if (SF6_GAS_METER != lMeterType)
	{
		GO(CalcLineEndPtStep2(hMemMgr, pHaarResponse, lRadius, lThreshold, *lineParam, minLength, &leftWidthConfidence, &rightWidthConfidence));
		JPrintf("******leftWidth=%d, rightWidth=%d\n", leftWidthConfidence, rightWidthConfidence);
	}
							...
							LINEPARAM lineParam, MLong minLength, MLong *leftWidthConfidence, MLong *rightWidthConfidence)
{
	...
	MLong lWidth, lHeight, lStride;
	MLong leftX, leftY, rightX, rightY, tmpX, tmpY;
	MLong tmpOff;
	...
	if (ptLeft.x==ptRight.x)
	{
		leftVal = pData + (ptMid.y-1) * lStride + ptMid.x;
		rightVal = pData + (ptMid.y+1) * lStride + ptMid.x;
		for (i=0; i<lWidthStep; i++, leftVal-=lStride, rightVal+=lStride)	
		{
			if (*leftVal >= lThreshold)
				leftWidthArrray[i]++;
			tmpVal1 = leftVal - 1;
			tmpVal2 = leftVal + 1;
			for (j=0; j<lMaxWidth; j++, tmpVal1--, tmpVal2++)
			{
	...
	else if (ptLeft.y==ptRight.y)
	{
		leftVal = pData + ptMid.y * lStride + ptMid.x - 1;
		rightVal = pData + ptMid.y * lStride + ptMid.x + 1;
		for (i=0; i<lWidthStep; i++, leftVal--, rightVal++)	
		{
			if (*leftVal >= lThreshold)
				leftWidthArrray[i]++;
			tmpVal1 = leftVal - lStride;
			tmpVal2 = leftVal + lStride;
			for (j=0; j<lMaxWidth; j++, tmpVal1-=lStride, tmpVal2+=lStride)
			{
				if(*tmpVal1>=lThreshold)
					leftWidthArrray[i]++;
				if(*tmpVal2>=lThreshold)
			...
			if(*rightVal >= lThreshold)
				rightWidthArray[i]++;
			tmpVal1 = rightVal - lStride;
			tmpVal2 = rightVal + lStride;
			for (j=0; j<lMaxWidth; j++, tmpVal1-=lStride, tmpVal2+=lStride)
			{
				if(*tmpVal1>=lThreshold)
					rightWidthArray[i]++;
	...
	for(yy=0; yy<srcHeight; yy++, pSrcData+=lSrcExt, xData+=lSrcExt, yData+=lSrcExt)
	{
        for(xx=0; xx<srcWidth; xx++, pSrcData++, xData++, yData++)
        {
            if(*xData>=0 && *xData<dstWidth && *yData>=0 && *yData<dstHeight)
            {
				...
				if((*pDstDataTmp&(~TEMPLATE_TYPE_LONG_BIT)) > (unsigned long)*lMaxVal)
...
{
	MRESULT res = LI_ERR_NONE;
	if (pCircleInfo==MNull) return LI_ERR_UNKNOWN;
...
//确定一个锚点，在锚点附件通过旋转和平移，求直线上响应最大的方式
//来求得最佳的直线
...
//	MDouble pt2lineDist[3];
	MDouble absDist[3];
	MLong direction[3];

	for (i=0; i<lPtNum; i++)
	{
		*(pPtDist + i) = vDistance3D(*(pPtPos+i), pLineParam->ptEnd);
		//*(pPtDist + i) = (MDouble)vPointDistToLine2(*(pPtPos+i), pLineParam->Coefk, pLineParam->Coefb, pLineParam->bVertical);
	}
...
//	pt2lineDist[0] = vPointDistToLine2D(*pPtPos, lineParam.Coefk, lineParam.Coefb, lineParam.bVertical);
//	pt2lineDist[1] = vPointDistToLine2D(*(pPtPos+1), lineParam.Coefk, lineParam.Coefb, lineParam.bVertical);
//	pt2lineDist[2] = vPointDistToLine2D(*(pPtPos+2), lineParam.Coefk, lineParam.Coefb, lineParam.bVertical);

	absDist[0] = vPointDistToLine2(*pPtPos, pLineParam->Coefk, pLineParam->Coefb, pLineParam->bVertical);
	absDist[1] = vPointDistToLine2(*(pPtPos+1), pLineParam->Coefk, pLineParam->Coefb, pLineParam->bVertical);
	absDist[2] = vPointDistToLine2(*(pPtPos+2), pLineParam->Coefk, pLineParam->Coefb, pLineParam->bVertical);

	tmpPt.y = (MLong)(pPtPos->x * pLineParam->Coefk + pLineParam->Coefb + 0.5);
	if(pPtPos->y < tmpPt.y)
		direction[0] = -1;
	else
		direction[0] = 1;
	tmpPt.y = (MLong)((pPtPos+1)->x * pLineParam->Coefk + pLineParam->Coefb + 0.5);
	if((pPtPos+1)->y < tmpPt.y)
		direction[1] = -1;
	else
		direction[1] = 1;
	tmpPt.y = (MLong)((pPtPos+2)->x * pLineParam->Coefk + pLineParam->Coefb + 0.5);
	if((pPtPos+2)->y < tmpPt.y)
		direction[2] = -1;
	else
		direction[2] = 1;
	...
	{
		result = (absDist[0]*pPtVal[1] + absDist[1]*pPtVal[0])/(absDist[0] + absDist[1] + WYRD_EPSILON);
		JPrintf("$$$$$$  dist1=%f, dist2=%f, res=%f\n", pPtDist[0], pPtDist[1], result);
	}
	else if (direction[0]*direction[2]<0)
	{
		result = (absDist[0]*pPtVal[2] + absDist[2]*pPtVal[0])/(absDist[0] + absDist[2] + WYRD_EPSILON);
		JPrintf("$$$$$$  dist1=%f, dist2=%f, res=%f\n", pPtDist[0], pPtDist[2], result);
	}
	else
	{
		result = pPtVal[0];
...
MDouble GetResultMethod2(MPOINT *pPtPos, MDouble *pPtVal, MDouble *pPtDist, LINEPARAM *pLineParam, MLong lPtNum)
{
	...
	for (i=0; i<lPtNum; i++)
	{
		*(pPtDist + i) = vPointDistToLine2(*(pPtPos+i), pLineParam->Coefk, pLineParam->Coefb, pLineParam->bVertical);
	}

	for (i=0; i<lPtNum-1; i++)
	{
		tmpPt.y = (MLong)((pPtPos+i)->x * pLineParam->Coefk + pLineParam->Coefb + 0.5);
		if((pPtPos+i)->y < tmpPt.y)
			direction[0] = -1;
		else
			direction[0] = 1;
		tmpPt.y = (MLong)((pPtPos+i+1)->x * pLineParam->Coefk + pLineParam->Coefb + 0.5);
		if((pPtPos+i+1)->y < tmpPt.y)
			direction[1] = -1;
		else
			direction[1] = 1;
		tmpVal = direction[0] * direction[1];
		if(tmpVal<=0)
			break;
	}
	...
	if (i == lPtNum-1)
	{
		if(*(pPtDist) < *(pPtDist+lPtNum-1))	// 起始刻度点距离直线最近
			result = *pPtVal;
		else
			result = *(pPtVal + i);
	}
	else
	{
		a = fabs(*(pPtDist + i));
		b = fabs(*(pPtDist + i + 1));
		result = (a*pPtVal[i+1] + b*pPtVal[i])/(a + b + WYRD_EPSILON);
		JPrintf("a=%f, b=%f, index=%d\n", a, b, i);
	}
...
MDouble GetResultMethod3(MPOINT *pPtPos, MDouble *pPtVal, MDouble *pPtDist, LINEPARAM *pLineParam, MLong lPtNum)
{
	...
	perVal = fabs(*(pPtVal+1) - *pPtVal);
	doublePerVal = 2 * perVal;
	...
	if (direction[0]*direction[1]<0)
	{
		if(pPtVal[1] > pPtVal[0]+doublePerVal)
			pPtVal[0] = pPtVal[1] + perVal;
		else if(pPtVal[0] > pPtVal[1]+doublePerVal)
			pPtVal[1] = pPtVal[0] + perVal;
		result = (absDist[0]*pPtVal[1] + absDist[1]*pPtVal[0])/(absDist[0] + absDist[1] + WYRD_EPSILON);
		JPrintf("$$$$$$  dist1=%f, dist2=%f, res=%f\n", pPtDist[0], pPtDist[1], result);
	}
	else if (direction[0]*direction[2]<0)
	{
		if(pPtVal[2] > pPtVal[0]+doublePerVal)
			pPtVal[0] = pPtVal[2] + perVal;
		else if(pPtVal[0] > pPtVal[2]+doublePerVal)
			pPtVal[2] = pPtVal[0] + perVal;
		result = (absDist[0]*pPtVal[2] + absDist[2]*pPtVal[0])/(absDist[0] + absDist[2] + WYRD_EPSILON);
		JPrintf("$$$$$$  dist1=%f, dist2=%f, res=%f\n", pPtDist[0], pPtDist[2], result);
	}
	else
...
MDouble GetSwitchStatus(MPOINT *pPtPos, LINEPARAM *pLineParam, MLong lPtNum)
{
	MDouble result;
...
MRESULT GetCircleRing(BLOCK *pHaarResponse, CIRCLE *pCircleParam, MLong lDelta, MLong lMeterType)
{
	...
	MLong lMaxRadius, lMinRadius, lCurDistance;
	...
	if (LEAKAGE_CURRENT_METER == lMeterType)
	{
		lMaxRadius = pCircleParam->lRadius - lDelta * 5;
		lMinRadius = pCircleParam->lRadius - lDelta * 11;	// 8 9 10	
	}
	else	// SF6
	{
