...
MVoid CreateGradAngleImg(const MUInt8 *pImgSrc, MInt32 lImgLine, MInt32 lWidth, MInt32 lHeight,MInt16 *pImgX, MInt16 *pImgY, MUInt16 *pMag, MUInt16 *pAngle)
{
    ...
    /*FILE *cannyFile;
	FILE *fp1=fopen("/tmp/sample/sX.txt", "w");
	FILE *fp2=fopen("/tmp/sample/sY.txt", "w");
	FILE *fp3=fopen("/tmp/sample/sMag.txt", "w");
	FILE *fp4=fopen("/tmp/sample/sAngle.txt", "w");
	*/
	MInt32 lMaxGrad = 0;
    MDouble angle;
	//printf("%x\n",pMag);
	...
	for (y=PROCESS_RADIUS; y<lHeight-PROCESS_RADIUS; y++)
	{
		...
		for (x=PROCESS_RADIUS; x<lWidth-PROCESS_RADIUS; x++)
		{
			...
			lMag=ABS(lGradX)+ABS(lGradY);
			//printf("%d %d %d\n",lGradX,lGradY,lMag);


			//lAngle = (MUInt16)(atan2((MDouble)lGradY,(MDouble)lGradX));
			//lAngle=GetAngle(lGradY,lGradX);
			//lAngle=atan(lGradY/(lGradX+0.1))/V_PI*12+12;

			if (lGradX==0)
			{
				angle=atan2(lGradY,lGradX+0.1)/V_PI*12;
			}
			else
			{
				angle=atan2(lGradY,lGradX)/V_PI*12;
			}
            if (angle>0)
            {
                lAngle = (MLong)(24-angle);
            }
            else
            {
                lAngle = (MLong)(-angle);
            }
			//fprintf(fp1,"%6d ",lGradX);
			//fprintf(fp2,"%6d ",lGradY);
			...
			//fprintf(fp3,"%6d ",pMagCur[x]);
			//pAngleCur[x] = (MUInt16)(((MUInt16)((lAngle+2*V_PI)*180/V_PI))%360);
			//pAngleCur[x]=(MUInt8)lAngle/15;
			pAngleCur[x]=(MUInt16)lAngle;//为了与硬件调试相对应
           // fprintf(fp4,"%6d ",pAngleCur[x]);
		}
		//fprintf(fp1,"\n");
		//fprintf(fp2,"\n");
		//fprintf(fp3,"\n");
   ...
   /* cannyFile = fopen("/tmp/sample/cannyB_mag.txt", "w");
		if(NULL==cannyFile)
		{
			printf("file open error\n");
		}

		for(ii=0;ii<lHeight;ii++)
		{
		   for(jj=0;jj<lImgLine;jj++)
		   {
			   fprintf(cannyFile, "%6d ", *pMag);
			   pMag++;
		   }
		   fprintf(cannyFile, "\n");
		}
...
#include "mpi_ive.h"
MVoid CreateGradAngle( const MUInt8 *pImgSrc, MInt32 lImgLine, MInt32 lWidth, MInt32 lHeight,MInt16 *pImgX, MInt16 *pImgY, MUInt16 *pMag, MUInt16 *pAngle)
{
