...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    layer = make_convolutional_layer(batch,h,w,c,n,size,stride,padding,activation, batch_normalize, binary, xnor, params.net.adam, use_bin_output);
	if (layer.errorcode < 0)
	{
		DEBUG_PRINT();
...
layer parse_yolo(list *options, size_params params)
{
	...
	{
		DEBUG_PRINT();
		printf("Error in parse_yolo_mask.\n");
		l.errorcode = -1;
		return l;
	}
	...
	{
		DEBUG_PRINT();
		printf("Error in make_yolo_layer.\n");
		return l;
	}
...
layer parse_reorg(list *options, size_params params)
{
    ...
    layer = make_reorg_layer(batch,w,h,c,stride,reverse);
	if (layer.errorcode < 0)
	{
		DEBUG_PRINT();
...
layer parse_shortcut(list *options, size_params params, network net)
{
	...
	if (s.errorcode < 0) {
		printf("Error in make_shortcut_layer.\n");
		return s;
	}
...
layer parse_upsample(list *options, size_params params, network net)
{
	...
	if (l.errorcode < 0)
	{
		printf("Error in make_upsample_layer.\n");
		return l;
	}
...
route_layer parse_route(list *options, size_params params, network net)
{
	...
	{
		DEBUG_PRINT();
		printf("Error in make_route_layer.\n");
		return layer;
	}
...
network parse_network_cfg_custom_no_gpu(char *filename, int batch)
{
	return parse_network_cfg_custom(filename, batch);
}
    ...
    while(n){
       ...
       // fprintf(stderr, "%4d ", count);
        ...
        if(lt == CONVOLUTIONAL){
			...
			{
				DEBUG_PRINT();
				printf("Error in parse_upsample.\n");
				net.errorcode = l.errorcode;
				free_section(s);
				free_list(sections);
				return net;
			...
			{
				DEBUG_PRINT();
				printf("Error in parse_shortcut.\n");
				net.errorcode = l.errorcode;
				free_section(s);
				free_list(sections);
				return net;
...
void load_weights_no_gpu(network *net, char *filename)
{
