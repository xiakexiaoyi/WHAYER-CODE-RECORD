...
int LightOffTrain(HYED_RESULT_LIST *resultlist)
{
	...
	resultlist->origin.x=(resultlist->DtArea[0].left+resultlist->DtArea[0].right)/2;//计算画框中心点
...
int LightOffRecog(HYED_RESULT_LIST resultlist)
{
	...
	pOrgImg2 = cvLoadImage("../../photo/IMG_20170602_105402.jpg", 1);//读取待测图
	if (!pOrgImg2)
	{
		printf("Error when loading image.\n"), exit(1); 
	}
	...
	for(int i=1;i < resultlist.lAreaNum;i++)//在图片中显示
	{
