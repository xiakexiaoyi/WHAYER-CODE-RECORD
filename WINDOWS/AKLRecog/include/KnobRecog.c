...
image HYKNOB_ipl_to_image(KR_IMAGES *pImg)
{
...
int HYKR_SetParam(void *hMRHandle,char *cfgfile, char *weightfile,float Thresh,int w,int h)
{
	int res=0;
	DRParam *pDRParam = (DRParam*)hMRHandle;
...
int knobcomp(const void *pa, const void *pb)
{
	float diff;
...
int knobboxcomp(const void *pa, const void *pb)
{
    float* a = *(float* *)pa;
    float* b = *(float* *)pb;
    float diff = a[4] - b[4] ;
...
void KNOBGetOverLappingRate(MKRECT rt1,MKRECT rt2,long *minRate, long *maxRate)
{
...
int HYKR_KnobRecog(void *hMRHandle,KR_IMAGES *pImg , HYKR_RESULT_LIST *pResultList)
{
	...
	qsort(bbox.Bbox, bbox.num, sizeof(float*), knobboxcomp);
	COM_free_image(out);
	...
	qsort(s, bbox.num, sizeof(sortable_KnobForm), knobcomp);  //从左到右排序,从上到下排序
...
//////////////////////////////////////////////////////////// //work
	for(j = 0; j < pResultList->lRealResultNum; ++j)
	{
		...
		for(i = 0; i < bbox.num; ++i){
			...
			KNOBGetOverLappingRate(targetarea,tmpArea,NULL,&tmpRate1);
			KNOBGetOverLappingRate(tmpArea,targetarea,NULL,&tmpRate2);
