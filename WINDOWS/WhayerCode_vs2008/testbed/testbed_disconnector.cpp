...
#ifdef _DEBUG
#pragma comment(lib, "..\\Debug\\disconnector.lib")
...
#define SOURCE_JPG "../all/201709291644411.jpg" 
...
int disconnectorTrain(HYED_RESULT_LIST *outPattern);	//训练
int disconnectorRecog(HYED_RESULT_LIST inPara);	//识别
//
int main(int argc, char** argv){
	...
	disconnectorTrain(&resultlist);
	disconnectorRecog(resultlist);
...
int disconnectorTrain(HYED_RESULT_LIST *resultlist)
{
	int res = 0;;
	...
	pOrgImg  = cvLoadImage(SOURCE_JPG, 1);//colord  基准图    
	if (!pOrgImg)
	{
...
int disconnectorRecog(HYED_RESULT_LIST resultlist)
{
	int res = 0;
	char modelfile[50];
	...
	pOrgImg2 = cvLoadImage(SOURCE_JPG, 1);
	if (!pOrgImg2)
	{
		printf("Error when loading image.\n"), exit(1); 
	}
	...
	if (HYL_disconnectorReg(NULL, &imgs, &resultlist)<0)//识别目标
	{
	...
	{
		if(resultlist.pResult[i].result == 0)
		{
			printf("开关%d状态:合\n",y+1);
		}
