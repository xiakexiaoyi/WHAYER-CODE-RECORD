...
#ifdef _DEBUG
#pragma comment(lib, "..\\Debug\\AirSwitch.lib")
#pragma comment(lib, "..\\Debug\\MatchRigidBody.lib")
#else
#pragma comment(lib, "..\\Release\\AirSwitch.lib")
...
int AirSwitchTrain(HYED_RESULT_LIST *outPattern);	//训练
int AirSwitchRecog(HYED_RESULT_LIST inPara);	//识别
//旋钮开关
int main(int argc, char** argv){
	...
	AirSwitchTrain(&resultlist);
	AirSwitchRecog(resultlist);
...
int AirSwitchTrain(HYED_RESULT_LIST *resultlist)
{
	int res = 0;;
	...
	pOrgImg  = cvLoadImage("..\\..\\Web\\CaptureFiles\\2017-03-01\\192.168.4.84_01_20170301170812531.jpg", 1);//colord  基准图    
	if (!pOrgImg)
	{
...
int AirSwitchRecog(HYED_RESULT_LIST resultlist)
{
	int res = 0;
	char modelfile[50];
	...
	HYL_AirSwitchInit(NULL, &KRHandle);
	HYL_MatchInit(NULL, &MHandle);
	...
	HYL_AirSwitchSetParam(KRHandle,modelfile,Filters0file,Filters1file);//分类器算法输入参数
	pOrgImg2 = cvLoadImage("..\\..\\Web\\CaptureFiles\\2017-03-01\\192.168.4.84_01_20170301170812531.jpg", 1);
	if (!pOrgImg2)
	{
		printf("Error when loading image.\n"), exit(1); 
	}
	...
	if (HYL_AirSwitchExceptionDetection(KRHandle, &imgs, &resultlist)<0)//识别目标
	{
	...
	for(int i=1;i < resultlist.lAreaNum;i++)
	{
		...
		else if(modeltype == 1)
		{
			...
			else if(resultlist.pResult[i].result == 3)
			{
				printf("开关%d状态:偏左\n",y+1);
			}
			else
