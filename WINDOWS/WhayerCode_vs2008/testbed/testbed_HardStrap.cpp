...
#ifdef _DEBUG
#pragma comment(lib, "..\\Debug\\HardStrap.lib")
#pragma comment(lib, "..\\Debug\\MatchRigidBody.lib")
#else
#pragma comment(lib, "..\\Release\\HardStrap.lib")
...
int HardStrapTrain(HYED_RESULT_LIST *outPattern);	//训练
int HardStrapRecog(HYED_RESULT_LIST inPara);	//识别
//旋钮开关
int main(int argc, char** argv){
	...
	HardStrapTrain(&resultlist);
	HardStrapRecog(resultlist);
...
int HardStrapTrain(HYED_RESULT_LIST *resultlist)
{
	int res = 0;;
	...
	pOrgImg  = cvLoadImage("..\\..\\Web\\CaptureFiles\\2017-03-01\\192.168.4.84_01_20170301180542180.jpg", 1);//colord  基准图    
	if (!pOrgImg)
	{
...
int HardStrapRecog(HYED_RESULT_LIST resultlist)
{
	int res = 0;
	char modelfile[50];
	...
	HYL_HardStrapInit(NULL, &KRHandle);
	HYL_MatchInit(NULL, &MHandle);
	...
	HYL_HardStrapSetParam(KRHandle,modelfile,Filters0file,Filters1file);//分类器算法输入参数
	pOrgImg2 = cvLoadImage("..\\..\\Web\\CaptureFiles\\2017-03-01\\192.168.4.84_01_20170301180542180.jpg", 1);
	if (!pOrgImg2)
	{
		printf("Error when loading image.\n"), exit(1); 
	}
	...
	if (HYL_HardStrapExceptionDetection(KRHandle, &imgs, &resultlist)<0)//识别目标
	{
