...
extern "C"
{
#include "ProtectionLink\ProtectionLink_SqueezeNet_S1.h"
}
#include "resample.h"
...
MRESULT HYL_ProtectionLinkInit(MHandle hMemMgr, MHandle *pMRHandle)
{
    ...
    pTLHandle->gaborsvm->gaborsize=20;
...
MRESULT   HYL_ProtectionLinkUninit(MHandle hMRHandle)
{
...
//test parameters 
MRESULT HYL_ProtectionLinkSetParam(MHandle hMRHandle,char *modelfile,char *Filters0file,char *Filters1file)
{
	MRESULT res = 1;
	TLParam *pTLHandle = (TLParam*)hMRHandle;
...
MRESULT HYL_ProtectionLinkExceptionDetection(MHandle hMRHandle,HYL_IMAGES *pImage,HYED_RESULT_LIST *pResultList)
{
	MDWord i, j, k;
	MLong lCurTime;
	MLong lMeanVal=0;
	...
	if (!pImage){res = LI_ERR_INVALID_PARAM; goto EXT;}
//	JASSERT(pImage->lHeight<=pImage2->lHeight && pImage->lWidth<=pImage2->lWidth);
	...
	GO(ImgCreate(pTLParam->hMemMgr, &ResizeImg, FORMAT_BGR, 227, 227));//FORMAT_BGR
	MRECT originArea;
	MRECT rtArea;
	for(int i=1;i<pResultList->lAreaNum;i++)
	{
		...
		W=rtArea.right-rtArea.left;
		H=rtArea.bottom-rtArea.top;
		//rtArea.left=rtArea.left-5;
		//rtArea.right=rtArea.right+5;
		//rtArea.top=rtArea.top-0.1*H;
		//rtArea.bottom=rtArea.bottom+0.1*H;
		if(rtArea.bottom == img.dwHeight)rtArea.bottom=rtArea.bottom-1;
		if(rtArea.right == img.dwWidth)rtArea.right=rtArea.right-1;
		if(rtArea.bottom >= img.dwHeight || rtArea.left < 0 || rtArea.top < 0 || rtArea.right >= img.dwWidth || rtArea.left>=rtArea.right ||rtArea.top>=rtArea.bottom)
		{
			res = -2;
			continue;
		}
		...
		ResampleImage(pTLParam->hMemMgr,imgT.dwWidth,imgT.dwHeight,imgT.pixelArray.chunky.dwImgLine,ResizeImg.dwWidth,ResizeImg.dwHeight,ResizeImg.pixelArray.chunky.dwImgLine,(unsigned char*)imgT.pixelArray.chunky.pPixel,(unsigned char*)ResizeImg.pixelArray.chunky.pPixel,0);
		...
		label=-1;
		ProtectionLink_SqueezeNet_S1_run_C3((unsigned char*)ResizeImg.pixelArray.chunky.pPixel,ResizeImg.dwWidth,ResizeImg.dwHeight,ResizeImg.pixelArray.chunky.dwImgLine,&label);
		if(label==0)
	...
	ImgRelease(pTLParam->hMemMgr,&ResizeImg);
