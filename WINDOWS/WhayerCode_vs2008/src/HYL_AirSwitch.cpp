...
MRESULT HYL_AirSwitchInit(MHandle hMemMgr, MHandle *pMRHandle)
{
...
MRESULT   HYL_AirSwitchUninit(MHandle hMRHandle)
{
...
//test parameters 
MRESULT HYL_AirSwitchSetParam(MHandle hMRHandle,char *modelfile,char *Filters0file,char *Filters1file)
{
	MRESULT res = 1;
	TLParam *pTLHandle = (TLParam*)hMRHandle;
...
MRESULT HYL_AirSwitchExceptionDetection(MHandle hMRHandle, HYL_IMAGES *pImage,HYED_RESULT_LIST *pResultList)
{
	MDWord i, j, k;
	MLong lCurTime;
	MLong lMeanVal=0;
