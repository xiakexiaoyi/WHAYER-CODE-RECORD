...
MRESULT HYL_KnobRegInit(MHandle hMemMgr, MHandle *pMRHandle)
{
...
//test parameters 
MRESULT HYL_KnobRegSetParam(MHandle hMRHandle,char *modelfile,char *Filters0file,char *Filters1file)
{
	MRESULT res = 1;
	TLParam *pTLHandle = (TLParam*)hMRHandle;
	pTLHandle->gaborsvm->model=svm_load_model(modelfile);
	pTLHandle->gaborsvm->model1=svm_load_model(Filters0file);
EXT:
	return res;
}


MRESULT HYL_KnobRegExceptionDetection(MHandle hMRHandle, HYL_IMAGES *pImage,HYED_RESULT_LIST *pResultList)
{
	MDWord i, j, k;
	MLong lCurTime;
	MLong lMeanVal=0;
...
//	JASSERT(pImage->lHeight<=pImage2->lHeight && pImage->lWidth<=pImage2->lWidth);
	...
	for(int i=1;i<pResultList->lAreaNum;i++)
	{
		...
		GaborSVMKnob(pTLParam->hMemMgr,gaborsvm,imgGray,&rtArea,&pResultList->pResult[i].result);
