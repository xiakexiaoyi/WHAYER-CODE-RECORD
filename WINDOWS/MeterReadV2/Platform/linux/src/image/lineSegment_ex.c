 ...
 */
static MLong double_equal(MDouble a, MDouble b)
{
  MDouble abs_diff,aa,bb,abs_max;
 ...
 */
static MDouble dist(MDouble x1, MDouble y1, MDouble x2, MDouble y2)
{
...
typedef struct ntuple_list_s
{
  ...
  MUInt32 dim;
  MDouble * values;
} * ntuple_list,ntuple_list_t;

/*----------------------------------------------------------------------------*/
/** Free memory used in n-tuple 'in'.
 */
static void free_ntuple_list(MHandle hMemMgr,ntuple_list in)
{
  //if( in == NULL || in->values == NULL )
 //   error("free_ntuple_list: invalid n-tuple input.");
  FreeVectMem(hMemMgr, in->values );
  //FreeVectMem( hMemMgr, in );
}
 ...
 */
MRESULT new_ntuple_list(MHandle hMemMgr,MUInt32 dim, ntuple_list n_tuple)
{
  //ntuple_list n_tuple;
  ...
  //AllocVectMem(hMemMgr,n_tuple,1,struct ntuple_list_s);
  //AllocVectMem(hMemMgr,n_tuple,1,ntuple_list_t);
  ...
  //n_tuple->values = (MDouble *) malloc( dim*n_tuple->max_size * sizeof(MDouble) );
  AllocVectMem(hMemMgr,n_tuple->values, dim*n_tuple->max_size,MDouble);
 ...
 */
MRESULT enlarge_ntuple_list(MHandle hMemMgr,ntuple_list n_tuple)
{
	MRESULT res=LI_ERR_NONE;
	MDouble *pValues;
  /* check parameters */
  ...
  /* realloc memory */
  //n_tuple->values = (MDouble *) realloc( (MVoid *) n_tuple->values,
    //                  n_tuple->dim * n_tuple->max_size * sizeof(MDouble) );

  pValues=n_tuple->values;
  n_tuple->values=MNull;
  AllocVectMem(hMemMgr,n_tuple->values, n_tuple->dim * n_tuple->max_size,MDouble);
  JMemCpy((MVoid*)(n_tuple->values),(MVoid*)(pValues),n_tuple->dim * n_tuple->max_size/2*sizeof(MDouble));

  //if( n_tuple->values == NULL ) error("not enough memory.");
  
EXT:
  FreeVectMem(hMemMgr,pValues);
  return res;
}
 ...
 */
static MRESULT add_7tuple(MHandle hMemMgr, ntuple_list out, MDouble v1, MDouble v2, MDouble v3,
                        MDouble v4, MDouble v5, MDouble v6, MDouble v7 )
{
  /* check parameters */
 MRESULT res=LI_ERR_NONE;
  /* if needed, alloc more tuples to 'out' */
  if( out->size == out->max_size ) GO(enlarge_ntuple_list(hMemMgr,out));
 ...
 */
static MVoid free_image_char(MHandle hMemMgr,image_char i)
{
  ...
  //free( (MVoid *) i );
	if (i!=NULL)
  FreeVectMem(hMemMgr,i->data);
  //FreeVectMem(hMemMgr,i);
}

/*----------------------------------------------------------------------------*/
/** Create a new image_char of size 'xsize' times 'ysize'.
 */
MRESULT new_image_char(MHandle hMemMgr,MUInt32 xsize, MUInt32 ysize,image_char image)
{
  //image_char image;
MRESULT res=LI_ERR_NONE;
  /* check parameters */
  ...
  //AllocVectMem(hMemMgr,image,1,struct image_char_s);
  //if( image == NULL ) error("not enough memory.");
  //image->data = (unsigned char *) calloc( (size_t) (xsize*ysize),
    //                                      sizeof(unsigned char) );
  AllocVectMem(hMemMgr,image->data,xsize*ysize,MByte);
 ...
 */
static image_char new_image_char_ini(MHandle hMemMgr, MUInt32 xsize, MUInt32 ysize,
                                      MByte fill_value,image_char image )
{
	MRESULT res=LI_ERR_NONE;
  //image_char image = new_image_char(xsize,ysize); /* create image */
  MUInt32 N = xsize*ysize;
  MUInt32 i;

  GO(new_image_char(hMemMgr,xsize,ysize,image));
  /* check parameters */
 ...
 */
//static image_int new_image_int(MUInt32 xsize, MUInt32 ysize)
MRESULT new_image_int(MHandle hMemMgr,MUInt32 xsize,MUInt32 ysize,image_int image)
{
  //image_int image;
  ...
  //AllocVectMem(hMemMgr,image,1,struct image_int_s);
  //if( image == NULL ) error("not enough memory.");
  //image->data = (int *) calloc( (size_t) (xsize*ysize), sizeof(int) );
  AllocVectMem(hMemMgr,image->data,xsize*ysize,MInt32);
  //if( image->data == NULL ) error("not enough memory.");
...
/** Create a new image_int of size 'xsize' times 'ysize',
    initialized to the value 'fill_value'.
 */
MRESULT new_image_int_ini(MHandle hMemMgr,MUInt32 xsize, MUInt32 ysize,
                                    MInt32 fill_value,image_int image )
{
  //image_int image = new_image_int(xsize,ysize); /* create image */
	MRESULT res=LI_ERR_NONE;
	MUInt32 N = xsize*ysize;
	MUInt32 i;
	GO(new_image_int(hMemMgr,xsize,ysize,image));
 ...
 */
static void free_image_double(MHandle hMemMgr,image_double i)
{
  //if( i == NULL || i->data == NULL )
 ...
 */
MRESULT new_image_double(MHandle hMemMgr,MUInt32 xsize, MUInt32 ysize,image_double image)
{
  ...
  //AllocVectMem (hMemMgr,image,1,struct image_double_s);
  //if( image == NULL ) error("not enough memory.");
  //image->data = (double *) calloc( (MUInt32) (xsize*ysize), sizeof(double) );
  AllocVectMem(hMemMgr,image->data,(xsize*ysize),MDouble);
  //if( image->data == NULL ) error("not enough memory.");
 ...
 */
static MVoid new_image_double_ptr(image_double image, MUInt32 xsize,
                                          MUInt32 ysize, MDouble * data )
{
...
/** Compute a Gaussian kernel of length 'kernel->dim',
    ...
    and 'kernel->values[1]'.
 */
static MRESULT gaussian_kernel(MHandle hMemMgr,ntuple_list kernel, MDouble sigma, MDouble mean)
{
	MRESULT res=LI_ERR_NONE;
  MDouble sum = 0.0;
  MDouble val;
  ...
  {
	  GO(enlarge_ntuple_list(hMemMgr,kernel));
  }
  kernel->size = 1;
  for(i=0;i<kernel->dim;i++)
    {
      val = ( (MDouble) i - mean ) / sigma;
...
/** Scale the input image 'in' by a factor 'scale' by Gaussian sub-sampling.
    ...
    in the y axis.
 */
static MRESULT gaussian_sampler(MHandle hMemMgr, image_double in, MDouble scale,
                                      MDouble sigma_scale,image_double out )
{
	MRESULT res=LI_ERR_NONE;
 
  struct image_double_s aux_tmp={0};
  image_double aux=&aux_tmp;
  struct ntuple_list_s kernel_tmp={0};
  ntuple_list kernel=&kernel_tmp;
  MUInt32 N,M,h,n,x,y,i;
  MInt32 xc,yc,j,double_x_size,double_y_size;
  MDouble sigma,xx,yy,sum,prec;

  /* check parameters */
 

  /* compute new image size and get memory for images */
  //if( in->xsize * scale > (MDouble) UINT_MAX || in->ysize * scale > (MDouble) UINT_MAX )
  //  error("gaussian_sampler: the output image size exceeds the handled size.");
  N = (MUInt32) ceil( in->xsize * scale );
  M = (MUInt32) ceil( in->ysize * scale );
  //GO(new_image_double(hMemMgr,N,in->ysize,aux_tmp));
  GO(new_image_double(hMemMgr,N,in->ysize,aux))
  //out = new_image_double(N,M);
  GO(new_image_double(hMemMgr,N,M,out));
   ...
   */
  prec = 3.0;
  h = (MUInt32) ceil( sigma * sqrt( 2.0 * prec * log(10.0) ) );
  n = 1+2*h; /* kernel size */
  //kernel = new_ntuple_list(n);
  GO(new_ntuple_list(hMemMgr,n,kernel));

  /* auxiliary double image size variables */
  double_x_size = (MInt32) (2 * in->xsize);
  double_y_size = (MInt32) (2 * in->ysize);
  ...
  for(x=0;x<aux->xsize;x++)
    {
      ...
      xc = (int) floor( xx + 0.5 );
      GO(gaussian_kernel(hMemMgr, kernel, sigma, (double) h + xx - (double) xc ));
          ...
          for(i=0;i<kernel->dim;i++)
            {
              ...
              if( j >= (MInt32) in->xsize ) j = double_x_size-1-j;
  ...
  for(y=0;y<out->ysize;y++)
    {
      ...
      yc = (MInt32) floor( yy + 0.5 );
      GO(gaussian_kernel(hMemMgr, kernel, sigma, (MDouble) h + yy - (MDouble) yc ));
          ...
          for(i=0;i<kernel->dim;i++)
            {
              ...
              if( j >= (MInt32) in->ysize ) j = double_y_size-1-j;
...
EXT:  /* free memory */
  free_ntuple_list(hMemMgr,kernel);
  free_image_double(hMemMgr,aux);
  return res;
}
 ...
 */
static MRESULT ll_angle(MHandle hMemMgr, image_double in, MDouble threshold,
                              struct coorlist ** list_p, MVoid ** mem_p,
                              image_double * modgrad, MUInt32 n_bins,image_double g)
{
  //image_double g;
	MRESULT res=LI_ERR_NONE;
  MUInt32 n,p,x,y,adr,i;
  MDouble com1,com2,gx,gy,norm,norm2;
  ...
  GO(new_image_double(hMemMgr,in->xsize,in->ysize,g));

  /* get memory for the image of gradient modulus */
  //*modgrad = new_image_double(in->xsize,in->ysize);
  GO(new_image_double(hMemMgr,in->xsize,in->ysize,*modgrad))

  /* get memory for "ordered" list of pixels */
  //list = (struct coorlist *) calloc( (size_t) (n*p), sizeof(struct coorlist) );
  AllocVectMem(hMemMgr,list,n*p,struct coorlist);
  SetVectMem(list,n*p,0,struct coorlist);
  *mem_p = (MVoid *) list;
  ...
  //range_l_e = (struct coorlist **) calloc( (size_t) n_bins,
    //                                       sizeof(struct coorlist *) );
   AllocVectMem(hMemMgr,range_l_s,n_bins,struct coorlist);
    SetVectMem(range_l_s,n_bins,0,struct coorlist);
	AllocVectMem(hMemMgr,range_l_e,n_bins,struct coorlist);
	SetVectMem(range_l_e,n_bins,0,struct coorlist);
    ...
    for(y=0;y<n-1;y++)
      {
        ...
        i = (MUInt32) (norm * (MDouble) n_bins / max_grad);
        ...
        range_l_e[i]->x = (MInt32) x;
        range_l_e[i]->y = (MInt32) y;
  ...
  *list_p = start;
  ...
  /* free memory */
  //free( (MVoid *) range_l_s );
  //free( (MVoid *) range_l_e );
EXT:
  FreeVectMem(hMemMgr,range_l_e);
  FreeVectMem(hMemMgr,range_l_s);
 ...
 */
static MInt32 isaligned( MInt32 x, MInt32 y, image_double angles, MDouble theta,
                      MDouble prec )
{
  MDouble a;
 ...
 */
static MDouble angle_diff(MDouble a, MDouble b)
{
  a -= b;
 ...
 */
static MDouble angle_diff_signed(MDouble a, MDouble b)
{
  a -= b;
  while( a <= -M_PI ) a += M_2__PI;
  while( a >   M_PI ) a -= M_2__PI;
...
/** Computes the natural logarithm of the absolute value of
    ...
    and
      ...
      q6 = 2.50662827511.
 */
static MDouble log_gamma_lanczos(MDouble x)
{
  static MDouble q[7] = { 75122.6331530, 80916.6278952, 36308.2951477,
                         8687.24529705, 1168.92649479, 83.8676043424,
                         2.50662827511 };
  MDouble a = (x+0.5) * log(x+5.5) - (x+5.5);
  MDouble b = 0.0;
  int n;
 ...
 */
static MDouble log_gamma_windschitl(MDouble x)
{
 ...
 */
static MDouble nfa(MInt32 n, MInt32 k, MDouble p, MDouble logNT)
{
  static MDouble inv[TABSIZE];   /* table to keep computed inverse values */
  MDouble tolerance = 0.1;       /* an error of 10% in the result is accepted */
  MDouble log1term,term,bin_term,mult_term,bin_tail,err,p_term;
  MInt32 i;
  ...
  if( n==k ) return -logNT - (MDouble) n * log10(p);
   ...
   */
  log1term = log_gamma( (MDouble) n + 1.0 ) - log_gamma( (MDouble) k + 1.0 )
           - log_gamma( (MDouble) (n-k) + 1.0 )
           + (MDouble) k * log(p) + (MDouble) (n-k) * log(1.0-p);
  term = exp(log1term);

  /* in some cases no more computations are needed */
  if( double_equal(term,0.0) )              /* the first term is almost zero */
    {
      if( (MDouble) k > (MDouble) n * p )     /* at begin or end of the tail?  */
  ...
  for(i=k+1;i<=n;i++)
    {
       ...
       */
      bin_term = (MDouble) (n-i+1) * ( i<TABSIZE ?
                   ( inv[i]!=0.0 ? inv[i] : ( inv[i] = 1.0 / (MDouble) i ) ) :
                   1.0 / (MDouble) i );
      ...
      if(bin_term<1.0)
        {
          ...
          err = term * ( ( 1.0 - pow( mult_term, (MDouble) (n-i+1) ) ) /
                         (1.0-mult_term) - 1.0 );
...
struct rect
{
  MDouble x1,y1,x2,y2;  /* first and second point of the line segment */
  MDouble width;        /* rectangle width */
  MDouble x,y;          /* center of the rectangle */
  MDouble theta;        /* angle */
  MDouble dx,dy;        /* (dx,dy) is vector oriented as the line segment */
  MDouble prec;         /* tolerance angle */
  MDouble p;            /* probability of a point with angle within 'prec' */
};
...
typedef struct
{
  MDouble vx[4];  /* rectangle's corner X coordinates in circular order */
  MDouble vy[4];  /* rectangle's corner Y coordinates in circular order */
  MDouble ys,ye;  /* start and end Y values of current 'column' */
  MInt32 x,y;       /* coordinates of currently explored pixel */
} rect_iter;
...
/** Interpolate y value corresponding to 'x' value given, in
    ...
    - x  <= x2
 */
static MDouble inter_low(MDouble x, MDouble x1, MDouble y1, MDouble x2, MDouble y2)
{
  /* check parameters */
...
/** Interpolate y value corresponding to 'x' value given, in
    ...
    - x  <= x2
 */
static MDouble inter_hi(MDouble x, MDouble x1, MDouble y1, MDouble x2, MDouble y2)
{
  /* check parameters */
 ...
 */
static MVoid ri_del(MHandle hMemMgr,rect_iter * iter)
{
  //if( iter == NULL ) error("ri_del: NULL iterator.");
  //free( (MVoid *) iter );
  FreeVectMem(hMemMgr,iter);
}
 ...
 */
static MInt32 ri_end(rect_iter * i)
{
  /* check input */
...
/** Increment a rectangle iterator.
    ...
    See details in \ref rect_iter
 */
static MVoid ri_inc(rect_iter * i)
{
  /* check input */
  ...
  while( (MDouble) (i->y) > i->ye && !ri_end(i) )
    {
      ...
      /* update lower y limit (start) for the new 'column'.
         ...
         by using 'inter_low'.
       */
      if( (MDouble) i->x < i->vx[3] )
        i->ys = inter_low((MDouble)i->x,i->vx[0],i->vy[0],i->vx[3],i->vy[3]);
      else
        i->ys = inter_low((MDouble)i->x,i->vx[3],i->vy[3],i->vx[2],i->vy[2]);
      ...
      /* update upper y limit (end) for the new 'column'.
         ...
         by using 'inter_low'.
       */
      if( (MDouble)i->x < i->vx[1] )
        i->ye = inter_hi((MDouble)i->x,i->vx[0],i->vy[0],i->vx[1],i->vy[1]);
      else
        i->ye = inter_hi((MDouble)i->x,i->vx[1],i->vy[1],i->vx[2],i->vy[2]);

      /* new y */
      i->y = (MInt32) ceil(i->ys);
    }
}
...
/** Create and initialize a rectangle iterator.
    ...
    See details in \ref rect_iter
 */
static rect_iter * ri_ini(MHandle hMemMgr,struct rect * r)
{
  MDouble vx[4],vy[4];
  MRESULT res=LI_ERR_NONE;
  MInt32 n,offset;
  ...
  AllocVectMem(hMemMgr,i,1,rect_iter);
  ...
  /* Set an initial condition.
     ...
     the first 'column'.
   */
  i->x = (MInt32) ceil(i->vx[0]) - 1;
  i->y = (MInt32) ceil(i->vy[0]);
  i->ys = i->ye = -DBL_MAX;
 ...
 */
static MDouble rect_nfa(MHandle hMemMgr, struct rect * rec, image_double angles, MDouble logNT)
{
  rect_iter * i;
  MInt32 pts = 0;
  MInt32 alg = 0;
  ...
  for(i=ri_ini(hMemMgr,rec); !ri_end(i); ri_inc(i)) /* rectangle iterator */
    if( i->x >= 0 && i->y >= 0 &&
        i->x < (MInt32) angles->xsize && i->y < (MInt32) angles->ysize )
      {
        ++pts; /* total number of pixels counter */
        if( isaligned(i->x, i->y, angles, rec->theta, rec->prec) )
          ++alg; /* aligned points counter */
      }
  ri_del(hMemMgr,i); /* delete iterator */
 ...
 */
static MDouble get_theta( struct point * reg, MInt32 reg_size, MDouble x, MDouble y,
                         image_double modgrad, MDouble reg_angle, MDouble prec )
{
  MDouble lambda,theta,weight;
  MDouble Ixx = 0.0;
  MDouble Iyy = 0.0;
  MDouble Ixy = 0.0;
  MInt32 i;
  ...
  for(i=0; i<reg_size; i++)
    {
      ...
      Ixx += ( (MDouble) reg[i].y - y ) * ( (MDouble) reg[i].y - y ) * weight;
      Iyy += ( (MDouble) reg[i].x - x ) * ( (MDouble) reg[i].x - x ) * weight;
      Ixy -= ( (MDouble) reg[i].x - x ) * ( (MDouble) reg[i].y - y ) * weight;
    }
 ...
 */
static MVoid region2rect( struct point * reg, MInt32 reg_size,
                         image_double modgrad, MDouble reg_angle,
                         MDouble prec, MDouble p, struct rect * rec )
{
  MDouble x,y,dx,dy,l,w,theta,weight,sum,l_min,l_max,w_min,w_max;
  MInt32 i;
  ...
  for(i=0; i<reg_size; i++)
    {
      weight = modgrad->data[ reg[i].x + reg[i].y * modgrad->xsize ];
      x += (MDouble) reg[i].x * weight;
      y += (MDouble) reg[i].y * weight;
      sum += weight;
    }
  ...
  for(i=0; i<reg_size; i++)
    {
      l =  ( (MDouble) reg[i].x - x) * dx + ( (MDouble) reg[i].y - y) * dy;
      w = -( (MDouble) reg[i].x - x) * dy + ( (MDouble) reg[i].y - y) * dx;
 ...
 */
static void region_grow( MInt32 x, MInt32 y, image_double angles, struct point * reg,
                         MInt32 * reg_size, MDouble * reg_angle, image_char used,
                         MDouble prec )
{
  MDouble sumdx,sumdy;
  MInt32 xx,yy,i;
  ...
  for(i=0; i<*reg_size; i++)
    for(xx=reg[i].x-1; xx<=reg[i].x+1; xx++)
      for(yy=reg[i].y-1; yy<=reg[i].y+1; yy++)
        if( xx>=0 && yy>=0 && xx<(MInt32)used->xsize && yy<(MInt32)used->ysize &&
 ...
 */
static MDouble rect_improve(MHandle hMemMgr, struct rect * rec, image_double angles,
                            MDouble logNT, MDouble log_eps )
{
  struct rect r;
  MDouble log_nfa,log_nfa_new;
  MDouble delta = 0.5;
  MDouble delta_2 = delta / 2.0;
  MInt32 n;

  log_nfa = rect_nfa(hMemMgr,rec,angles,logNT);
  ...
  for(n=0; n<5; n++)
    {
      r.p /= 2.0;
      r.prec = r.p * M_PI;
      log_nfa_new = rect_nfa(hMemMgr,&r,angles,logNT);
 ...
 */
static MInt32 reduce_region_radius( struct point * reg, MInt32 * reg_size,
                                 image_double modgrad, MDouble reg_angle,
                                 MDouble prec, MDouble p, struct rect * rec,
                                 image_char used, image_double angles,
                                 MDouble density_th )
{
  MDouble density,rad1,rad2,rad,xc,yc;
  MInt32 i;
  ...
  while( density < density_th )
    {
      ...
      for(i=0; i<*reg_size; i++)
        if( dist( xc, yc, (MDouble) reg[i].x, (MDouble) reg[i].y ) > rad )
          {
 ...
 */
static MInt32 refine( struct point * reg, MInt32 * reg_size, image_double modgrad,
                   MDouble reg_angle, MDouble prec, MDouble p, struct rect * rec,
                   image_char used, image_double angles, MDouble density_th )
{
  MDouble angle,ang_d,mean_angle,tau,density,xc,yc,ang_c,sum,s_sum;
  MInt32 i,n;
  ...
  for(i=0; i<*reg_size; i++)
    {
      used->data[ reg[i].x + reg[i].y * used->xsize ] = NOTUSED;
      if( dist( xc, yc, (MDouble) reg[i].x, (MDouble) reg[i].y ) < rec->width )
        {
  ...
  mean_angle = sum / (MDouble) n;
  tau = 2.0 * sqrt( (s_sum - 2.0 * mean_angle * sum) / (MDouble) n
...
/** LSD full interface.
 */
MRESULT  LineSegmentDetection(MHandle hMemMgr, MInt32 * n_out,
                               MDouble * img, MInt32 lLine,MInt32 X, MInt32 Y,
                               MDouble scale, MDouble sigma_scale, MDouble quant,
                               MDouble ang_th, MDouble log_eps, MDouble density_th,
                               MInt32 n_bins,
                               MInt32 ** reg_img, MInt32 * reg_x, MInt32 * reg_y ,MDouble **return_values)
{
  image_double image;
  MRESULT res=LI_ERR_NONE;
  ntuple_list out =MNull;
  ntuple_list_t out_temp={0};
  MDouble * return_value;
  MDouble *imageData,*imageDataTmp;
  
  struct image_double_s scaled_image_temp={0};
  struct image_double_s angles_temp={0};
  struct image_double_s modgrad_temp={0};

  image_double scaled_image=&scaled_image_temp,angles=&angles_temp,modgrad=&modgrad_temp;

  struct image_char_s used_temp={0};
  image_char used=&used_temp;
  struct image_int_s region_temp={0};
  image_int region = MNull;
  MLong i,j;
  MLong lExt=lLine-X;
  ...
  struct rect rec;
  struct point * reg;
  MInt32 reg_size,min_reg_size;
  MUInt32 xsize,ysize;
  MDouble rho,reg_angle,prec,p,log_nfa,logNT;
  MInt32 ls_count = 0;                   /* line segments are numbered 1,2,3,... */
  ...
  /*allocate ntuple_list*/
  GO(new_ntuple_list(hMemMgr,7,&out_temp));
  out=&out_temp;


  /*translate the MUInt* image to MDouble* image*/
  

  /* load and scale image (if necessary) and compute angle at each pixel */
  AllocVectMem(hMemMgr,image,1,struct image_double_s);
  new_image_double_ptr(image,(MUInt32) X, (MUInt32) Y, img);

  if( scale != 1.0 )
    {
      //scaled_image = gaussian_sampler(image, scale, sigma_scale );
	  GO(gaussian_sampler(hMemMgr,image,scale,sigma_scale,scaled_image));
      //angles = ll_angle( scaled_image, rho, &list_p, &mem_p,
        //                 &modgrad, (MUInt32) n_bins );
	  GO(ll_angle(hMemMgr,scaled_image,rho, &list_p, &mem_p,
		                   &modgrad, (MUInt32) n_bins ,angles));
     // free_image_double(scaled_image);//ÔÚ×îºó
    }
  else
    //angles = ll_angle( image, rho, &list_p, &mem_p, &modgrad,
      //                 (MUInt32) n_bins );
	  GO(ll_angle(hMemMgr,image,rho, &list_p, &mem_p,
	  &modgrad, (MUInt32) n_bins ,angles));
  xsize = angles->xsize;
  ysize = angles->ysize;
  ...
  */
  logNT = 5.0 * ( log10( (MDouble) xsize ) + log10( (MDouble) ysize ) ) / 2.0
          + log10(11.0);
  min_reg_size = (MInt32) (-logNT/log10(p)); /* minimal number of points in region
  ...
  if( reg_img != NULL && reg_x != NULL && reg_y != NULL ) /* save region data */
	...
	GO(new_image_int_ini(hMemMgr,angles->xsize,angles->ysize,0,region));
  //used = new_image_char_ini(xsize,ysize,NOTUSED);
  GO(new_image_char_ini(hMemMgr,xsize,ysize,NOTUSED,used));
  //reg = (struct point *) calloc( (size_t) (xsize*ysize), sizeof(struct point) );
  AllocVectMem(hMemMgr,reg,xsize*ysize,struct point);
  ...
  for(; list_p != NULL; list_p = list_p->next )
    if( used->data[ list_p->x + list_p->y * used->xsize ] == NOTUSED &&
          ...
          because we are only interested in the exact NOTDEF value */
      {
        ...
        /* compute NFA value */
        log_nfa = rect_improve(hMemMgr,&rec,angles,logNT,log_eps);
        ...
        GO(add_7tuple(hMemMgr,out, rec.x1, rec.y1, rec.x2, rec.y2,
...
EXT:
  /* free memory */
  free_image_double(hMemMgr,scaled_image);
  FreeVectMem(hMemMgr,image);
  //free( (MVoid *) image );  
  /* only the double_image structure should be freed,
                               the data pointer was provided to this functions
                               and should not be destroyed.                 */
  free_image_double(hMemMgr,angles);
  free_image_double(hMemMgr,modgrad);
  free_image_char(hMemMgr,used);
  //free( (MVoid *) reg );
  //free( (MVoid *) mem_p );
  FreeVectMem(hMemMgr,reg);
  FreeVectMem(hMemMgr,mem_p);
  ...
  if( reg_img != NULL && reg_x != NULL && reg_y != NULL )
    {
      ...
      *reg_img = region->data;
     // if( region->xsize > (MUInt32) INT_MAX ||
      //    region->xsize > (MUInt32) INT_MAX )
      //  error("region image to big to fit in INT sizes.");
      *reg_x = (MInt32) (region->xsize);
      *reg_y = (MInt32) (region->ysize);
      ...
      //free( (MVoid *) region );
	  FreeVectMem(hMemMgr,region);
    }
  //if( out->size > (MUInt32) INT_MAX )
   // error("too many detections to fit in an INT.");
  *n_out = (MInt32) (out->size);

  //return_value = out->values;
  *return_values = (out->values);
 // free( (void *) out ); 
