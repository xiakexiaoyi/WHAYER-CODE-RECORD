...
//	JPrintf("CreateLineSet~~~\n");
	{
		...
		{
		    maxDist2Center = MAX(5, (pCircleInfo->circleParam.lRadius*7)>>5);	//1/4 ?  1/2 ?	1/8?
			
		}
		else
		{
            maxDist2Center = 10000;
			for(j=0;j<seedTmp.lSeedNum; j++)
			{
				tmpR.x +=seedTmp.pptSeed->x;
				tmpR.y +=seedTmp.pptSeed->y;
				...
				while(bChanged)
				{
					...
					for (m = 0 ; m<seedTmp.lSeedNum; m++)
					{
						...
						for(n = 0;n<numInPoint;n++)
						{
							...
							if(distmp>=tmpCircleParam.lRadius*3/2)
								break;
						}
						if(distmp>=tmpCircleParam.lRadius*3/2 && numInPoint>0)
							continue;*/
						{
							pTmpPt[numInPoint].x = seedTmp.pptSeed[m].x;
							pTmpPt[numInPoint++].y = seedTmp.pptSeed[m].y;
					...
					if(numInPoint1==0||(dCoeffK1!=dCoeffK || dCoeffB1!=dCoeffB || bVert1!=bVert)||flag)
					{
						if (numInPoint>numInPoint1)
						{
							bChanged = MTrue;
	...
	else
	{
		...
		if (k<1.0 && k>-1.0)
		{
			...
			for (xx=lStart; xx<=lEnd; xx++)
			{
				...
				for( i= -3;i<=3;i++)
				{
					lTmpVal = (*(pTmpData + xx + (yy+i) * lStride)) & lMask;
					if (lTmpVal > 0)
					{
						lStep++;
					}
