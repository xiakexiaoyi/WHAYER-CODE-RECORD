	...
	LINEPARAM lineParam, MLong minLength, MLong *leftWidthConfidence, MLong *rightWidthConfidence)
{
	...
	if (ptLeft.x==ptRight.x)
	{
		...
		AllocVectMem(hMemMgr, pMem, lWidthStep*lMaxWidth*2, MPOINT);
		pValueList = (MByte *)pMem;
		...
		GO(QuickSort(hMemMgr, pMem, lCounts, sizeof(MByte),ValueCmptmp));
		lThreshold=*((MByte*)pMem+lPercentage*lCounts/100);
		FreeVectMem(hMemMgr,pMem);
		///////////////////new threshold/////////////////////
		pData = (MByte*)pHaarResponse->pBlockData;
		leftX = ptMid.x;
		leftY = ptMid.y - 1;
	...
	else if (dCoeffK<1.0 && dCoeffK>-1.0)	// x
	{
		...
		leftX = ptMid.x - 1;
		rightX = ptMid.x + 1;
		AllocVectMem(hMemMgr, pMem, lWidthStep*lMaxWidth*4, MPOINT);
		pValueList = (MByte *)pMem;
		for (i=0; i<lWidthStep; i++, leftX--, rightX++)
		{
			leftY = (MLong)(dCoeffK * leftX + dCoeffB);
		...
		//printf("********lThreshold_old=%d\n",lThreshold);
		GO(QuickSort(hMemMgr, pMem, lCounts, sizeof(MByte),ValueCmptmp));
		lThreshold=*((MByte*)pMem+lPercentage*lCounts/100);
		FreeVectMem(hMemMgr,pMem);
		//printf("********lThreshold_new=%d\n",lThreshold);
		/////////////////new threshold//////////////////////
		pData = (MByte*)pHaarResponse->pBlockData;
		leftX = ptMid.x - 1;
		rightX = ptMid.x + 1;
