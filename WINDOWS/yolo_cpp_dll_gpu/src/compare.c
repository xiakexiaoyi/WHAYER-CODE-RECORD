...
void train_compare(char *cfgfile, char *weightfile)
{
    ...
    while(1){
        ...
        printf("%.3f: %f, %f avg, %lf seconds, %d images\n", (float)*net.seen/N, loss, avg_loss, sec(clock()-time1), *net.seen);
        free_data(train);
        if(i%100 == 0){  
...
void validate_compare(char *filename, char *weightfile)
{
	...
	network net;
	list *plist;
	int imgs,N,epoch,i=0,correct,total,splits,num;
	int j,k;
	matrix pred;
	pthread_t load_thread;
    data val;
    ...
    for(i = 1; i <= splits; ++i){
        ...
        free_matrix(pred);
        printf("%d: Acc: %f, %lf seconds, %d images\n", i, (float)correct/total, sec(clock()-time1), val.X.rows);
        free_data(val);
    }
}
...
int bbox_comparator(const void *a, const void *b)
{
	...
	float *X ;
	float *predictions;
	sortable_bbox box1,box2;
    ++total_compares;
    box1 = *(sortable_bbox*)a;
    box2 = *(sortable_bbox*)b;
    net = box1.net;
    classes   = box1.class;

    im1 = load_image_color(box1.filename, net.w, net.h);
    im2 = load_image_color(box2.filename, net.w, net.h);
    X  = (float*)calloc(net.w*net.h*net.c, sizeof(float));
    ...
    free_image(im2);
    free(X);
    if (predictions[classes*2] > predictions[classes*2+1]){
        return 1;
    }
    return -1;
}
...
void bbox_update(sortable_bbox *a, sortable_bbox *b, int class, int result)
{
    float EA,EB,SA,SB;
    int k = 32;
    EA = 1./(1+pow(10, (b->elos[class] - a->elos[class])/400.));
    EB = 1./(1+pow(10, (a->elos[class] - b->elos[class])/400.));
    SA = result ? 1 : 0;
    SB = result ? 0 : 1;
...
void bbox_fight(network net, sortable_bbox *a, sortable_bbox *b, int classes, int class)
{
    ...
    for(i = 0; i < classes; ++i){
        if(class < 0 || class == i){
			int result ;
            result = predictions[i*2] > predictions[i*2+1];
...
void SortMaster3000(char *filename, char *weightfile)
{
    ...
    N = plist->size;
    free_list(plist);
    boxes = (sortable_bbox *)calloc(N, sizeof(sortable_bbox));
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
    ...
    for (classtmp = 0; classtmp < classes; ++classtmp){
		FILE *outfp;
		char buff[256];
        N = total;
        current_class = classtmp;
        ...
        for(round = 1; round <= 100; ++round){
            ...
            for(i = 0; i < N/2; ++i){
                bbox_fight(net, boxes+i*2, boxes+i*2+1, classes, classtmp);
            }
            qsort(boxes, N, sizeof(sortable_bbox), elo_comparator);
        ...
        sprintf(buff, "results/battle_%d.log", classtmp);
        outfp = fopen(buff, "w");
        for(i = 0; i < N; ++i){
            fprintf(outfp, "%s %f\n", boxes[i].filename, boxes[i].elos[classtmp]);
        }
        fclose(outfp);
    }
