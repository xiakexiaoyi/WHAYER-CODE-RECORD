...
void random_go_moves(moves m, float *boards, float *labels, int n)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        flip = rand()%2;
        rotate = rand()%4;
        in = float_to_image(19, 19, 1, boards+i*19*19);
        out = float_to_image(19, 19, 1, labels+i*19*19);
        if(flip){
            flip_image(in);
            flip_image(out);
        }
...
void train_go(char *cfgfile, char *weightfile)
{
    ...
    board = calloc(19*19*net.batch, sizeof(float));
    move = calloc(19*19*net.batch, sizeof(float));
    m = load_go_moves("/home/pjreddie/backup/go.train");
    ...
    while(get_current_batch(net) < net.max_batches || net.max_batches == 0){
        ...
        loss = train_network_datum(net, board, move) / net.batch;
...
void predict_move(network net, float *board, float *move, int multi)
{
    ...
    if(multi){
        ...
        for(i = 1; i < 8; ++i){
            ...
            output = network_predict(net, board);
            oim = float_to_image(19, 19, 1, output);
...
int generate_move(network net, int player, float *board, int multi, float thresh, float temp, char *ko, int print)
{
    ...
    max = max_index(move, 19*19);
    row = max / 19;
    col = max % 19;
    index = sample_array(move, 19*19);
...
void valid_go(char *cfgfile, char *weightfile, int multi)
{
    ...
    board = calloc(19*19, sizeof(float));
    move = calloc(19*19, sizeof(float));
    m = load_go_moves("/home/pjreddie/backup/go.test");
...
void engine_go(char *filename, char *weightfile, int multi)
{
    ...
    set_batch_network(&net, 1);
    board = calloc(19*19, sizeof(float));
    one = calloc(91, sizeof(char));
    two = calloc(91, sizeof(char));
    ...
    while(1){
        ...
        if (!strcmp(buff, "protocol_version")){
            ...
            char comm[256];
            scanf("%s", comm);
            known = (!strcmp(comm, "protocol_version") || 
                    !strcmp(comm, "name") || 
                    !strcmp(comm, "version") || 
            ...
            index = generate_move(net, player, board, multi, .1, .7, two, 1);
            if(passed || index < 0){
                printf("=%s pass\n\n", ids);
                passed = 0;
            } else {
...
void test_go(char *cfg, char *weights, int multi)
{
    ...
    while(1){
        ...
        dnum = sscanf(line, "%d", &picked);
        cnum = sscanf(line, "%c", &c);
...
void self_go(char *filename, char *weightfile, char *f2, char *w2, int multi)
{
    ...
    while(1){
        ...
        //print_board(board, 1, 0);
        //sleep(1);
        use = ((total%2==0) == (player==1)) ? net : net2;
        index = generate_move(use, player, board, multi, .1, .7, two, 0);
        if(index < 0){
            done = 1;
            continue;
        }
...
void run_go(int argc, char **argv)
{
    ...
    c2 = (argc > 5) ? argv[5] : 0;
    w2 = (argc > 6) ? argv[6] : 0;
    multi = find_arg(argc, argv, "-multi");
