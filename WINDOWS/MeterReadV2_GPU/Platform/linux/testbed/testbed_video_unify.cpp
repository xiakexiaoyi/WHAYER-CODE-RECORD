...
//#define METER_TYPE  HY_VOLT_METER
...
//	cvNamedWindow("testImage",0);
	...
	for (i=0; i<pOutPattern->lLineNum; i++)
	{
		...
		if (1==lTmpVal)
		{
			pOutPattern->lLineColor[i] = LINE_BLACK;
		}
		else if (2==lTmpVal)
		{
			pOutPattern->lLineColor[i] = LINE_WHITE;
		}
		else
		{
			pOutPattern->lLineColor[i] = LINE_RED;
		}
	}
	...
	WY_DEBUG_PRINT("请在指针线上标记3个点用于计算线宽（包含指针线两端）:\n");
	lPtNumTmp = 0;
	while(1)
	{
		if (CV_EVENT_RBUTTONDOWN == mouseParam[2])	// add red circle
		...
		cvShowImage("TrainImage",testImage);
		if(13==cvWaitKey(10) || lPtNumTmp>=3)	// Enter
			break;
	}
	
	HYAMR_CalcHaarWidth(hHYMRDHand, &wy_testImage, pOutPattern->lLineColor, ptListTmp, lPtNumTmp, &lTmpVal);
	pOutPattern->lLineWidth = lTmpVal;

	WY_DEBUG_PRINT("请设计表盘MASK类型：（1.矩形 2.圆形 3.圆环 4.圆环+直线 5.其他）\n");
	scanf("%d", &lTmpVal);
	switch(lTmpVal)
	{
		case 1:
