						   ...
						   MDouble dThreshold, unsigned char *pCurTargetMem, unsigned char *pTrainTargetMem, HYAMR_READ_PARA *pLinePara)
{
	...
	for (i=0; i<pPara->lLineNum; i++)
	{
		...
		if (pPara->lLineNum >= 2)
		{
			if (LINE_RED == pPara->lLineColor[i])
			{
		...
		else if (LINE_RED == pPara->lLineColor[i])		// one line case
		{
			GO(B_Create(pMgr->hMemMgr, &diffBlockImage, DATA_U8, srcBlock.lWidth, srcBlock.lHeight));
			GO(ImgCreate(pMgr->hMemMgr, &ImgSrcYUV420, FORMAT_YUV420_VUVU, srcImg.dwWidth, srcImg.dwHeight));
			GO(ImgFmtTrans(&srcImg, &ImgSrcYUV420));
		...
		{
			ReadParaInfoCpy(inParam, pTmpLine);
		}
	}
...
//=========================================================
MRESULT HYAMR_SaveTargetInfoToMem (MHandle hHYMRDHandle, unsigned char *pCurTargetMem, unsigned char *pTrainTaregtMem, MBool isTrain)
{
	MRESULT res = LI_ERR_NONE;
...
MRESULT HYAMR_SetParam(MHandle hHYMRDHandle, HYLPAMR_IMAGES pMaskImage, HYAMR_INTERACTIVE_PARA *pInPara)
{
	...
	lLeft = pMgr->ptTrainTarget.left;
	lTop = pMgr->ptTrainTarget.top;
...
//根据指针左右边界点 计算指针宽度
MRESULT HYAMR_CalcHaarWidth(MHandle hHYMRDHandle, HYLPAMR_IMAGES pSrcImg, LINE_TYPE *pLineColor, MPOINT *pPtList, MLong lPtNum, MLong *lHaarWidth)
{
	MRESULT res = LI_ERR_NONE;
	...
	if (LINE_RED == pLineColor[0])
	{
		...
		PrintBmpEx(grayBlockImg.pBlockData, grayBlockImg.lBlockLine, DATA_U8, grayBlockImg.lWidth, grayBlockImg.lHeight, 1, "D:\\yuv2gray.bmp");
	}
	else
	{
	...
	lExt = lStride - lWidth;
	lSearchThreshold = MAX(5, (MIN(lWidth, lHeight))>>4);

	GO(B_Create(pMgr->hMemMgr, &edgeImg, DATA_U8, lWidth, lHeight));
	if (LINE_RED == pLineColor[0])
	{
		pEdgeData = edgeImg.pBlockData;
		...
		PrintBmpEx(edgeImg.pBlockData, edgeImg.lBlockLine, DATA_U8, edgeImg.lWidth, edgeImg.lHeight, 1, "D:\\edge_aa.bmp");
		for (i=0; i<lPtNum; i++)
		{
	...
	*lHaarWidth = 0;
	...
	{
		tmpPt = *(pPtList + i);
		searchPt(&edgeImg, tmpPt, 0, lSearchThreshold, &ptDirection[0], &lDirectionLength[0]);
		...
		// 更合理的做法 还需要对返回的边缘点ptDirection做下判断 是否为指针线边缘
		*lHaarWidth += MAX(4, MIN(lTmpWidthArray[0], MIN(lTmpWidthArray[1], MIN(lTmpWidthArray[2], lTmpWidthArray[3]))));
	}
	*lHaarWidth /= lPtNum;
