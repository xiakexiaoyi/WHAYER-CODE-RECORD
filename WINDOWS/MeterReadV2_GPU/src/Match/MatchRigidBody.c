	...
	*/
	...
	if(lParam==1)//对图像进行旋转处理
	{
		...
		for (i=0;i<36;i++)
		{
			...
			//PrintBmpEx(imageRotate.pBlockData, imageRotate.lBlockLine,imageRotate.typeDataA & ~0x100,imageRotate.lWidth, imageRotate.lHeight, 1, ".\\log\\imageRotate2.bmp");
			RotateImage_EX(pMask,&maskRotate,10*i);
			//PrintBmpEx(maskRotate.pBlockData, maskRotate.lBlockLine,maskRotate.typeDataA & ~0x100,maskRotate.lWidth, maskRotate.lHeight, 1, ".\\log\\maskRotate2.bmp");
...
//    PrintBmpEx(pMask->pBlockData, pMask->lBlockLine,pMask->typeDataA & ~0x100,pMask->lWidth, pMask->lHeight, 1, ".\\log\\maskimage.bmp");
...
//	PrintBmpEx(operateImage.pBlockData, operateImage.lBlockLine,operateImage.typeDataA & ~0x100,operateImage.lWidth, operateImage.lHeight, 1, ".\\log\\operateimg.bmp");
	...
	*(ptDescriptorClass->ptDescriptor+ptDescriptorClass->lNumDescriptors-1)=tDescriptor;
		...
		//B_Release(hMemMgr,&imageMean);
		//PrintBmpEx(pImage->pBlockData, pImage->lBlockLine,pImage->typeDataA & ~0x100,pImage->lWidth, pImage->lHeight, 1, ".\\log\\image.bmp");
		
		//PrintBmpEx(pMask->pBlockData, pMask->lBlockLine,pMask->typeDataA & ~0x100,pMask->lWidth, pMask->lHeight, 1, ".\\log\\mask.bmp");
		//PrintBmpEx(maskRotate.pBlockData, maskRotate.lBlockLine,maskRotate.typeDataA & ~0x100,maskRotate.lWidth, maskRotate.lHeight, 1, ".\\log\\maskRotate.bmp");
		//B_Release(hMemMgr,&maskRotate);
		//B_Release(hMemMgr,&imageRotate);
	
//	}
	//WriteTDescriptorNum(".\\log\\descriptor.txt",tDescriptorNum);



	/*
	WriteMInt16Data(".\\log\\gx.txt",pImgX,pImage->lBlockLine,pImage->lWidth,pImage->lHeight);
	WriteMInt16Data(".\\log\\gy.txt",pImgY,pImage->lBlockLine,pImage->lWidth,pImage->lHeight);
	WriteMUInt16Data(".\\log\\Mag.txt",pMag,pImage->lBlockLine,pImage->lWidth,pImage->lHeight);
	WriteMUInt16Data(".\\log\\Angle.txt",pAngle,pImage->lBlockLine,pImage->lWidth,pImage->lHeight);
	*/
...
MRESULT GetOperateImage(MHandle hMemMgr,BLOCK *pSrc,BLOCK *pDst,MRECT rect,MLong lSample)
{
	...
	//PrintBmpEx(pImage.pBlockData, pImage.lBlockLine,pImage.typeDataA & ~0x100,pImage.lWidth, pImage.lHeight, 1, ".\\log\\operate.bmp");
	...
	//PrintBmpEx(pDst->pBlockData,pDst->lBlockLine,pDst->typeDataA& ~0x100,pDst->lWidth,pDst->lHeight,1,".\\log\\operate2.bmp");
...
MRESULT create_template_fast(MHandle hMemMgr,	BLOCK * pImage, MLong a_grad_num,MDouble a_thres ,TDescriptor * pDescriptor)
{
	...
	*/
	...
	//WriteMUInt8Data(".\\log\\templateData2.txt",lp_peak_tmp,N,N,M);
...
MRESULT ComputeGradients(MHandle hMemMgr,BLOCK *pImage,MLong lSample,MUInt8 **GradientData)
{
	...
	*/
	...
	//WriteMInt16Data(".\\log\\imggx.txt",pImgX,pImage->lBlockLine,pImage->lWidth,pImage->lHeight);
	//WriteMInt16Data(".\\log\\imggy.txt",pImgY,pImage->lBlockLine,pImage->lWidth,pImage->lHeight);
	//WriteMUInt16Data(".\\log\\imgMag.txt",pMag,pImage->lBlockLine,pImage->lWidth,pImage->lHeight);
	//WriteMUInt16Data(".\\log\\imgAngle.txt",pAngle,pImage->lBlockLine,pImage->lWidth,pImage->lHeight);
	...
	//WriteMUInt8Data(".\\log\\imgData.txt",lp_peak_tmp,);
	*GradientData=lp_peak_tmp;
	//cannyFile= fopen(".\\log\\opeateImg.txt", "w");
	...
	//fclose(cannyFile);
	//------------------------------------------------------------------------------
	//WriteMUInt8Data(".\\log\\imgData.txt",lp_peak_tmp,l_width,l_width,l_height);
	//for (x=0;)
...
MRESULT MatchRigidBody(MHandle hMemMgr,BLOCK *pImage,PTDescriptorClass ptDescriptorClass,MDouble threshold,PTOutParam ptOutParam)
{
	...
	*/
	...
	//PrintBmpEx(imageMean.pBlockData, imageMean.lBlockLine,imageMean.typeDataA & ~0x100,imageMean.lWidth, imageMean.lHeight, 1, ".\\log\\imageMean.bmp");
	...
	//PrintBmpEx(imageEdge.pBlockData, imageEdge.lBlockLine,imageEdge.typeDataA & ~0x100,imageEdge.lWidth, imageEdge.lHeight, 1, ".\\log\\imageEdge.bmp");
	...
	for (i=0;i<ptDescriptorClass->lNumDescriptors;i++)
	{
		...
		if (lX!=-1&&lY!=-1)
		{
			...
			//PrintBmpEx(pImage->pBlockData, pImage->lBlockLine,pImage->typeDataA & ~0x100,pImage->lWidth, pImage->lHeight, 1, ".\\log\\result2.bmp");
...
MVoid GetTemp(MHandle hMemMgr,BLOCK *pImage,TOutParam tOutParam)
{
	...
	PrintBmpEx(RectImg.pBlockData, RectImg.lBlockLine,RectImg.typeDataA & ~0x100,RectImg.lWidth, RectImg.lHeight, 1, ".\\log\\rect.bmp");
	...
	PrintBmpEx(edgeImg.pBlockData, edgeImg.lBlockLine,edgeImg.typeDataA & ~0x100,edgeImg.lWidth, edgeImg.lHeight, 1, ".\\log\\edge2.bmp");
	RotateImage(hMemMgr,&RectImg,&rotateImg,360-tOutParam.lIndex*10);
	PrintBmpEx(rotateImg.pBlockData, rotateImg.lBlockLine,rotateImg.typeDataA & ~0x100,rotateImg.lWidth, rotateImg.lHeight, 1, ".\\log\\rotate2.bmp");
	...
	PrintBmpEx(edgeImg2.pBlockData, edgeImg2.lBlockLine,edgeImg2.typeDataA & ~0x100,edgeImg2.lWidth, edgeImg2.lHeight, 1, ".\\log\\edge3.bmp");
