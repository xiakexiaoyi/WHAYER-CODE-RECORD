...
MRESULT GetLineInfo(MHandle hMemMgr,BLOCK *pImage, BLOCK *pMask, PARAM_INFO *pParam)
{
    ...
    //获取响应图
    PrintBmpEx(pImage->pBlockData, pImage->lBlockLine, DATA_U8, pImage->lWidth, pImage->lHeight, 1, ".\\log\\gray_aaa.bmp");
//	printf("stride=%d, width=%d, height=%d\n", pImage->lBlockLine, pImage->lWidth, pImage->lHeight);
	...
	PrintBmpEx(haarResult.pBlockData, haarResult.lBlockLine, DATA_U8, haarResult.lWidth, haarResult.lHeight, 1, ".\\log\\response_aaa.bmp");
	...
	PrintBmpEx(haarResult.pBlockData, haarResult.lBlockLine, DATA_U8, haarResult.lWidth, haarResult.lHeight, 1, ".\\log\\response_bbb.bmp");

	if (PIC_HIGH_BLUR == pParam->lPicBlurLevel)
	{
		inverseImg(&haarResult);
		PrintBmpEx(haarResult.pBlockData, haarResult.lBlockLine, DATA_U8, haarResult.lWidth, haarResult.lHeight, 1, ".\\log\\response_ccc.bmp");
	...
	if(pParam->lDoubleRedLine)
	{
		...
		PrintBmpEx(haarResult.pBlockData, haarResult.lBlockLine, DATA_U8, haarResult.lWidth, haarResult.lHeight, 1, ".\\log\\response_ddd.bmp");
	}
	...
	PrintBmpEx(blockFlag.pBlockData, blockFlag.lBlockLine, DATA_U8, blockFlag.lWidth, blockFlag.lHeight, 1,".\\log\\localMaxPre_bbb.bmp");
	JPrintf("localMax---------\n");
	//Logger("local max ok...\n");
	...
	PrintBmpEx(blockFlag.pBlockData, blockFlag.lBlockLine, DATA_U8, blockFlag.lWidth, blockFlag.lHeight, 1,".\\log\\localMaxPost.bmp");
...
//	JPrintf("CreateLineSet~~~\n");
	{
		...
		for (j=0; j<seedTmp.lSeedNum; j++)
		{
			...
			for (k=j+1; k<seedTmp.lSeedNum; k++)
			{
				...
				randomPoint[1].x = seedTmp.pptSeed[k].x;
				randomPoint[1].y = seedTmp.pptSeed[k].y;
				/*dis = sqrt((MDouble)(randomPoint[1].x-randomPoint[0].x)*(randomPoint[1].x-randomPoint[0].x)+
					(randomPoint[1].y-randomPoint[0].y)*(randomPoint[1].y-randomPoint[0].y));
				if(dis>=tmpCircleParam.lRadius<<1)
					continue;	*/
				...
				while(bChanged)
				{
					...
					for (m = 0 ; m<seedTmp.lSeedNum; m++)
					{
						...
						{
							distmp = sqrt((MDouble)(seedTmp.pptSeed[m].x-pTmpPt[n].x)*(seedTmp.pptSeed[m].x-pTmpPt[n].x)+
								(seedTmp.pptSeed[m].y-pTmpPt[n].y)*(seedTmp.pptSeed[m].y-pTmpPt[n].y));
							if(distmp>=tmpCircleParam.lRadius<<1)
								break;
						}
						if(distmp>=tmpCircleParam.lRadius<<1)
							continue;*/
...
//					pLineInfo->lConfidence, lMeanSum, lTotalPtNum, lTotalSumNum);
		...
		else if(lineSetInfo.lLineNum == 0 && pParam->lDoubleRedLine==0)
		{
...
MRESULT getBlackDiffImage2_yuv(MByte *pSrcY, MByte *pSrcV, MByte *pSrcU, MLong lSrcStride, BLOCK *pDiffImage, MLong *pHistArray)
{
	...
	lMaxNum = (lWidth * lHeight)>>1;     // >>3  (0.4 whb)
