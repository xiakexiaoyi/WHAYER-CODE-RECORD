...
MTChar g_szTextName[64] = _MMT(".\\log\\li_debug.txt");
...
MVoid PrintBmp(const MVoid *pImgData, MInt32 lImgLine, JTYPE_DATA_A typeDataA,
		MInt32 lWidth, MInt32 lHeight, MInt32 lChannelNum)
{
//	PrintBmpEx(pImgData, lImgLine, typeDataA, lWidth, lHeight, lChannelNum, ".\\log\\li_debug.bmp");
}
#endif	//ENABLE_DEBUG
...
#define PRINT_STR(str, filename)	WriteLogFile("S.\\log\\DSC00001.txt", str, strlen(str))
...
{
	MTChar *szLogName = ".\\log\\logger_log.txt";
	MTChar buf[1024];
