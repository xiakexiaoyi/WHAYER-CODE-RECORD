...
image HYLINK_ipl_to_image(LR_IMAGES *pImg)
{
...
int HYLR_SetParam(void *hMRHandle,char *cfgfile, char *weightfile,float Thresh,int w,int h)
{
	int res=0;
	DRParam *pDRParam = (DRParam*)hMRHandle;

	pDRParam->lThresh = Thresh;
	pDRParam->net = COM_parse_network_cfg_adapt(cfgfile,w,h);
...
int linkcomp(const void *pa, const void *pb)
{
	float diff;
...
int linkboxcomp(const void *pa, const void *pb)
{
    float* a = *(float* *)pa;
    float* b = *(float* *)pb;
    float diff = a[4] - b[4] ;
...
void LINKGetOverLappingRate(MLRECT rt1,MLRECT rt2,long *minRate, long *maxRate)
{
...
int HYLR_LinkRecog(void *hMRHandle,LR_IMAGES *pImg , HYLR_RESULT_LIST *pResultList)
{
	...
	qsort(bbox.Bbox, bbox.num, sizeof(float*), linkboxcomp);
	COM_free_image(out);
	...
	qsort(s, bbox.num, sizeof(sortable_LinkForm), linkcomp);  //从左到右排序,从上到下排序
	...
	for(j = 0; j < bbox.num; ++j){ 
		...
		for(i = 0; i < bbox.num; ++i){
			...
			LINKGetOverLappingRate(tmpArea,targetarea,NULL,&tmpRate1);
			LINKGetOverLappingRate(targetarea,tmpArea,NULL,&tmpRate2);
