...
image HYAIR_ipl_to_image_GPU(AR_IMAGES *pImg)
{
...
image HYAIR_ipl_to_imageROI_GPU(AR_IMAGES *pImg,int srcx,int srcy,int srcw,int srch)
{
	unsigned char *data = (unsigned char *)pImg->pixelArray.chunky.pPixel;
	int h = pImg->lHeight;
	int w = pImg->lWidth;
	...
	int step = pImg->pixelArray.chunky.lLineBytes;
	//int step = srcw*3;
    image out = COM_make_image_GPU(srcw, srch, c);
	int i, j, k, count=0;
	if(out.data==NULL)
		return out;
...
int HYAR_Init_GPU(void *hMemMgr, void **pMRHandle)
{
	int res = 0;
...
int HYAR_Uninit_GPU(void *hMRHandle)
{
...
int HYAR_SetParam_GPU(void *hMRHandle,char *cfgfile, char *weightfile,float Thresh,int gpu_index,int w,int h)
{
	int res=0;
	DRParam *pDRParam = (DRParam*)hMRHandle;
#ifdef GPU
...
int aircomp_GPU(const void *pa, const void *pb)
{
	float diff;
...
int airboxcomp_GPU(const void *pa, const void *pb)
{
    float* a = *(float* *)pa;
    float* b = *(float* *)pb;
    float diff = a[4] - b[4] ;
...
int roicomp_GPU(const void *pa, const void *pb)
{
	float diff;
    MARECT a = *(MARECT *)pa;
    MARECT b = *(MARECT *)pb;
...
void AIRGetOverLappingRate_GPU(MARECT rt1,MARECT rt2,long *minRate, long *maxRate)
{
...
int HYAR_AirSwitchRecog_GPU(void *hMRHandle,AR_IMAGES *pImg , HYAR_RESULT_LIST *pResultList)
{
	int res =0;
	int i=0,j=0;
	int total=100;
	...
	qsort(ROI, type, sizeof(float*),roicomp_GPU);//空开排数从上到下排序
	...
	for(i=1;i<type;i++)//单排空开识别
	{
		...
		if(_y < 0)
			_y=0;
		out = HYAIR_ipl_to_imageROI_GPU(pImg,_x,_y,_w,_h);
		if(out.data==NULL)
		{
			res=-1;
			goto EXT;
		...
		if(0!= COM_YoloDetector_GPU(pDRParam->net, out, thresh,&tmpbbox.num,tmpbbox.Bbox))
		{
			res=-1;
	...
	qsort(bbox.Bbox, bbox.num, sizeof(float*), airboxcomp_GPU);
	...
	qsort(s, bbox.num, sizeof(sortable_AirSwitchForm), aircomp_GPU);  //从左到右排序,从上到下排序
	for(j = 0; j < pResultList->lRealResultNum; ++j)
	{
		...
		for(i = 0; i < bbox.num; ++i){
			...
			AIRGetOverLappingRate_GPU(tmpArea,targetarea,NULL,&tmpRate1);
			AIRGetOverLappingRate_GPU(targetarea,tmpArea,NULL,&tmpRate2);
