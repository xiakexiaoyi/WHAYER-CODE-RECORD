...
int main123()
{
	...
	if ((fHandle = _findfirst(to_search, &fileinfo)) == -1L)
	{
		...
		do {
			...
			char *weightfile = "../1106/tiny-yolo-voc_final (2).weights";
			//char *cfgfile="../model/0402/tiny-yolo-voc.cfg";
			//char *weightfile="../model/0402/tiny-yolo-voc_47000.weights";
			int fileOrNot = 2;
			int len = strlen(fileinfo.name);
			sprintf(single_img, "%s\\%s", single_picname, fileinfo.name);
			fileinfo.name[len - 4] = '\0';
			pImg = cvLoadImage(single_img, CV_LOAD_IMAGE_COLOR);
			...
			{
				sprintf(result_img, "%s\\result\\%s_result_%f.jpg", single_picname, fileinfo.name, result.dConfidence);
			}
			cvSaveImage(result_img, pImg);
			//printf("%s\n", text);
			//cvSaveImage("result.jpg", pImg);
