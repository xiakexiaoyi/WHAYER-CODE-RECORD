...
void save_image_png_v3(image im, const char *name)
{
	char buff[256];
...
int yolo_detector_v3(const network net, const image pImg, float thresh, int *numDST, float **boxout)
{
	...
	int letterbox = 0;
	float hier_thresh = 0.5;
	image sized = resize_image(pImg, net.w, net.h);  //cost time

	//image sized = letterbox_image(pImg, net.w, net.h); letterbox = 1;

	//save_image_png_v3(sized,"sized");
	...
	time = what_time_is_it_now();
	network_predict(net, X);
	//network_predict_image(&net, pImg); letterbox = 1;
	...
	//box_detections_v3_letterbox(pImg, dets, nboxes, thresh, numDST, boxout);
