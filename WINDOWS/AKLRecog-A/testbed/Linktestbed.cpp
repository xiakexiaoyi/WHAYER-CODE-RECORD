...
#else
#pragma comment(lib, "../x64/Release/LinkRecog.lib")
...
int main()
{
	...
	//while(1)
	{
		...
		//fp = fopen("E:/WhayerCode/RecogByYolo/AKLRecog/Vresult.txt", "a");
		...
		//printf("%sstart:size=%d,resident=%d,share=%d\n",tmpbuf,procmem.size,procmem.resident,procmem.share);
		//fprintf(fp,"%sstart:size=%d,resident=%d,share=%d\n",tmpbuf,procmem.size,procmem.resident,procmem.share);*/
		...
		tt=time(NULL);
		strftime(tmpbuf,80,"%Y-%m-%d %H:%M:%S\n",localtime(&tt));
		printf("%send:size=%d,resident=%d,share=%d\n\n",tmpbuf,procmem.size,procmem.resident,procmem.share);
		fprintf(fp,"%send:size=%d,resident=%d,share=%d\n\n",tmpbuf,procmem.size,procmem.resident,procmem.share);
		fclose(fp);
		if(p_sigar)
		{
			sigar_close(p_sigar);
