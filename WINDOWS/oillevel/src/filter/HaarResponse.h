...
*************************************************/
	...
	typedef struct  
	{
		...
		MLong k;//Haar模板中间黑带的宽度
		MLong L;//Haar模板中间黑带离bottom的高度
		MLong lClassNumber;
		MLong lMethod;
		MLong *pAngleList;
		MLong lAngleNumber;
...
#define TEMPLATE_TYPE_BYTE_BIT		0x80
	...
	typedef struct
	{
		LINE_INFO *pLineInfo;//圆信息
		MVoid *pMem;//used to store the allocated memory
		MLong lLineNum;//当前的候选圆个数
		MLong lMaxLineNum;//最大的候选圆个数
	}LINE_SET_INFO;
	typedef struct 
	{
		// circle
...
#define GetPixelLocation               PX(GetPixelLocation)
