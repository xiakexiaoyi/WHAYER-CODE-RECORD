...
MRESULT ProbabilisticHoughTransform(MHandle hMemMgr, MByte* pBinData, MLong lSrcLine,
    ...
    MLong lLinesMax, LineSegment* pLines)  //ForeFlag为二值化图像白色数值 lThreshold为线段点个数阈值
{
    MRESULT res=LI_ERR_NONE;
    MLong *pAccum = 0;
    MLong *pTmpAccum;
...
{
	 return fabs(k*s->x-s->y+b)/sqrt(k*k+1);
}
double dis_3D(MPOINT* a,MPOINT* b,MPOINT* s)
{
	...
	double bs=sqrt(pow((b->x-s->x),2.0)+pow((b->y-s->y),2.0));
...
//	MByte *pTmpBinData;
    ...
    for (count = total; count>0; count--)
    {
        ...
        if( lGoodLine )
        {
            ...
            (pLines->EndPt+lAccountNum)->x = Line[1].x;
            (pLines->EndPt+lAccountNum)->y = Line[1].y;
			if (Line[0].x > 3.0*lWidth / 5)//开头要在图片前半部分
			{
				continue;
			}
			//if (Line[1].x - Line[0].x != 0)
			//{
			//	linek = 1.0*(Line[1].y - Line[0].y) / (Line[1].x - Line[0].x);
			//}
			//if (fabs(linek) > 0.25)//近似水平
			//	break;
			if(pLines->lLineNum > 0)
			{
				for(k=0;k<pLines->lLineNum;k++)//线之间的距离要大于一定值
				{
					distance=dis_3D((pLines->EndPt+k),(pLines->StartPt+k),(pLines->StartPt+lAccountNum))
						+dis_3D((pLines->EndPt+k),(pLines->StartPt+k),(pLines->EndPt+lAccountNum));
					if(distance < lMinLineDistance*2)
					{
						lAccountNum=lAccountNum-1;
						break;
					}
				}
			}
