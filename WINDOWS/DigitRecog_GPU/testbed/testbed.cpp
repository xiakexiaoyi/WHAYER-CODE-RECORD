...
#else
#pragma comment(lib, "../x64/Release/DigitalRecog_GPU.lib")
...
int DigitalRecog(HYDR_RESULT_LIST resultlist)
{
	...
	if(0!=HYDR_Init_GPU(NULL,&hTLHandle))
	{
	...
	int h = 0;
	int gpu_index = 0;
	if(0!=HYDR_SetParam_GPU(hTLHandle,cfgfile,weightfile,thresh, gpu_index,w,h))
	{
